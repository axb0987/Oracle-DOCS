...
OAuth 2.0 Client Credentials are not available in the Oracle UK Sovereign Cloud (OC4).
...
Each user can have up to 10 OAuth 2.0 client credentials. You can increase this limit by[Creating a Limit Increase Request](https://docs.oracle.com/iaas/Content/General/service-limits/create-request.htm).
...
- Create the OAuth 2.0 client credentials. See[To create OAuth 2.0 client credentials](https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/managingcredentials.htm#create-oauth20).
...
- Using the information from the previous step, make a request against the`/oauth2/token`endpoint to obtain a token as follows:
...
To create, change, and delete IAM database user names, see[Working with IAM Database User Names](https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/../Concepts/usercredentials_topic-iam_database_user_names.htm).
...
The IAM database password complexity uses almost the same rules supported in IAM for Console passwords (no double quotes ["] in IAM passwords). For details, see[Creating an IAM database password](https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/../Concepts/usercredentials_topic-creating_a_database_password.htm).

### Failed Login Lockout s

For information about failed logins, see[IAM Database Password Lockouts](https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/../Concepts/usercredentials_topic-password_lockouts.htm).
...
- Sign in to the Console using the Oracle Cloud Infrastructure User Name and Password.
- 

After you sign in, select the Profile menu on the upper-right side of the navigation bar at the top of the page, and then select Change password .
- Enter the Current Password .
- Enter your new password in the New Password and Confirm New Password fields, and then select Save New Password .
...
- View the user's details: Select the name of the identity domain that you want to work in. You might need to change the compartment to find the domain that you want. Then, select Users . Locate the user in the list, and then select the user's name to view the details.
- 

Select Create/Reset Password .
...
- If you're adding an API key for yourself : In the navigation menu , select the Profile menu and then select User settings .
...
- Click Add API Key .
- In the dialog, select Generate API Key Pair .
- 
...
- View the user's details:
- If you're getting an API key config file snippet for yourself : In the navigation menu , select the Profile menu and then select User settings .
...
- View the user's details:
- If you're creating an auth token for yourself: In the navigation menu , select the Profile menu and then select User settings .
...
- On the left side of the page, select Auth Tokens .
...
- View the user's details:
- If you're deleting an auth token for yourself : In the navigation menu , select the Profile menu and then select User settings .
- If you're an administrator deleting an auth token for another user : Open the navigation menu and select Identity &amp; Security . Under Identity , select Users . Locate the user in the list, and then select the user's name to view the details.
- On the left side of the page, select Auth Tokens .
- For the auth token you want to delete, select Delete .
...
- View the user's details:
- If you're creating a Customer Secret key for yourself: In the navigation menu , select the Profile menu and then select User settings .
...
On the left side of the page, select Customer Secret Keys .
...
Enter a friendly description for the key and select Generate Secret Key .
...
Copy the Secret Key immediately, because you can't retrieve the Secret Key again after closing the dialog box for security reasons.
...
- View the user's details:
- If you're deleting a Customer Secret key for yourself : In the navigation menu , select the Profile menu and then select User settings .
- If you're an administrator deleting a Customer Secret key for another user : Open the navigation menu and select Identity &amp; Security . Under Identity , select Users . Locate the user in the list, and then select the user's name to view the details.
- On the left side of the page, select Customer Secret Keys .
- For the Customer Secret key you want to delete, select Delete .
- Confirm when prompted.
...
- View the user's details:
- If you're creating an OAuth 2.0 client credential for yourself: In the navigation menu , select the Profile menu and then select User settings .
...
On the left side of the page, select OAuth 2.0 Client Credentials .
...
The following procedure works for a regular user or an administrator. Administrators can delete an auth token for either another user or themselves.
- View the user's details:
- If you're deleting an OAuth 2.0 Client Credential for yourself : In the navigation menu , select the Profile menu and then select User settings .
- If you're an administrator deleting an auth token for another user : Open the navigation menu and select Identity &amp; Security . Under Identity , select Users . Locate the user in the list, and then select the user's name to view the details.
- On the left side of the page, select OAuth 2.0 Client Credentials .
- For the OAuth 2.0 Client Credential you want to delete, select Delete .
- Confirm when prompted.
...
- View the user's details:
- If you're generating SMTP credentials for yourself : In the navigation menu , select the Profile menu and then select User settings .
- If you're an administrator generating SMTP credentials for another user : Open the navigation menu and select Identity &amp; Security . Under Identity , select Users . Locate the user in the list, and then click the user's name to view the details.
...
Copy the user name and password for your records and click Close . Copy the credentials immediately, because you can't retrieve the password again after closing the dialog box for security reasons.
...
- View the user's details:
- If you're deleting SMTP credentials for yourself : In the navigation menu , select the Profile menu and then select User settings .
- If you're an administrator deleting SMTP credentials for another user : Open the navigation menu and select Identity &amp; Security . Under Identity , select Users . Locate the user in the list, and then select the user's name to view the details.
- On the left side of the page, select SMTP Credentials .
- For the SMTP credentials you want to delete, select Delete .
- Confirm when prompted.
...
You can create an IAM database password to meet the Oracle Database password-creation guidelines.
- Log in to the OCI IAM console.
- In the upper right corner of the window, select the profile icon to display your user profile page.
- In your user profile page, select your user name.
- Under Resources , select Database Passwords.
- In the Database Passwords section, select Create Database Password.

The Create Database Password dialog box is displayed.
- Enter a description of the password.
- Note the password guidelines and restrictions listed on the page. See[Creating an IAM database password](https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/../Concepts/usercredentials_topic-creating_a_database_password.htm)for more information about password rules.
- Select Create Database Password.
...
- In your user profile page, select your user name.
- Under Resources , select Database Passwords.
- Your user name is displayed in the Database Passwords section.
- At the right end of the row with your user name in it, select the three-dot menu , and then select Delete .
...
- In the table of database users, select Create User .
- In the Name field, enter your database user name. Enter only letters, numerals, hyphens, periods, underscores, +, and @. You cannot use spaces in the name.
- In the Description field, optionally enter the name of the database that this user name is for or any other relevant information.
- Optionally select Advanced Options to show the Tags dialog box.
- In the Tag Namespace, Tag Key, and Value fields, enter a tag name
- Select Save Changes .
...
- Select Identity&amp; Security .
- Under Identity , select Users .
- In the table of database users, locate your database user name and left select it.

The page for your user name is displayed.
- Select Edit User .
- In the Description field, edit your database user name and select Save Changes .
...
The page for your user name is displayed.
- At the right end of the row that contains your user name select the three-dot menu and then select Delete .
