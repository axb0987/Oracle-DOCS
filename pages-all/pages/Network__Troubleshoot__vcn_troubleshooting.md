# VCN Troubleshooting
- Source: https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/vcn_troubleshooting.htm
...
Network security groups (NSGs) act as a virtual firewall for your DNS resolver endpoints. An NSG consists of a set of ingress and egress[security rules](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Concepts/securityrules.htm)that apply only to the associated DNS resolver endpoints.

## Secondary IP Address

If you assigned a secondary IP to a[secondary VNIC](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Tasks/managingVNICs.htm), and you're using policy-based routing for the secondary VNIC, configure the route rules for the instance to look up the same route table for the secondary IP address, using the`ip rule add from <source address> lookup <table name>`command.

## IP CIDR Addresses

If you assign an IP CIDR address to a VNIC and receive the following error message:

`“This operation is not valid for subnets without flexible CIDR feature enablement or prefix without flexible CIDR enabled”`

You can resolve this issue in one of the following ways:
- Add a new IPv6 network to the subnet: Add an IPv6 prefix to the subnet to enable the required flexible CIDR feature.
- Create a new subnet: Create a subnet with flexible CIDR enabled.

This error occurs because the subnet's IPv6 prefix was created before the IP CIDR address feature was introduced.
...
See[About the DNS Domains and Hostnames](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Concepts/dns.htm#About)for more information.
...
- Under Metrics , four tables are relevant to connection tracking:
...
- Move to a larger compute shape that has[higher connection limit](https://docs.oracle.com/iaas/Content/General/service-limits/default.htm#computecapacityreservations)
...
Remember:
- To delete a VCN, it must first be empty and have no related resources or attached gateways (for example: no[internet gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Tasks/managingIGs.htm),[dynamic routing gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Tasks/managingDRGs.htm), and so on).
...
- OCI database systems (Exadata Database Service on Dedicated Infrastructure, Autonomous AI Database on Dedicated Exadata Infrastructure, or Base Database systems)
Note  
  
When you create one of the preceding resources, you specify a VCN and subnet for it. The relevant service creates at least one[VNIC](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Tasks/managingVNICs.htm)in the subnet and attaches the VNIC to the resource. The service manages the VNICs on your behalf, so they are not readily apparent to you in the Console. The VNIC enables the resource to communicate with other resources over the network. Although this documentation commonly talks about the resource itself being in the subnet, it's actually the resource's attached VNIC. This documentation uses the term parent resource to refer to this type of resource.
...
Details An LPG created for a specific local peering connection can't be repurposed to be used in a new and different local peering connection. If you attempt to do this you might see the error message`The Local Peering Gateway with ID <LPG_OCID> has already been connected`when:
- LPG 1 in VCN 1 was once successfully connected to LPG 2 in VCN 2.
- VCN 1 (including LPG 1) is subsequently deleted. The LPG 2 peering status changes to revoked or destroyed.
- You try to connect LPG 2 to LPG 3.

You could also see a slightly different message (`A peering with VCN <VCN_OCID> has already been established`) when:
- LPG 1 in VCN 1 was once successfully connected to LPG 2 in VCN 2.
- LPG 2 is subsequently deleted. The LPG 1 peering status changes to revoked or destroyed.
- You try to connect LPG 1 to LPG 3 (which could be a new LPG in VCN 2 or any other VCN). Suggested Solution Perform the following actions:
- Delete the LPG you're attempting to repurpose.
...
- Create a new LPG and peer it with a fresh LPG in the desired VCN.

## Other useful links

- [Virtual Network Interface Cards (VNICs)](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Tasks/managingVNICs.htm)
- [DNS in a Virtual Cloud Network](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Concepts/dns.htm)
- [IP Management](https://docs.oracle.com/en-us/iaas/Content/Network/Troubleshoot/../Concepts/ipaddressesanddns.htm)
