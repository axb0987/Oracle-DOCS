...
- Source: https://docs.oracle.com/en-us/iaas/Content/Registry/Concepts/registryconcepts.htm
- Fetched: 2026-09-05 02:53 CDT

# Container Registry Concepts

Find out about the key concepts you need to understand before using Container Registry.
...
A Docker image can also be uniquely identified by its image digest, which is a hash created from the image’s content and metadata. Unlike a version identifier, which can point to different images over time, the digest always refers to the exact same image and ensures consistent deployments.
...
- `us-phoenix-1.ocir.io/cbujx0t3wa3r/my-hello-app`registry domain
...
For the list of region identifiers and region keys, see[Availability by Region](https://docs.oracle.com/en-us/iaas/Content/Registry/Concepts/registryprerequisites.htm#regional-availability). region identifier
...
For the list of region identifiers, see[Availability by Region](https://docs.oracle.com/en-us/iaas/Content/Registry/Concepts/registryprerequisites.htm#regional-availability). region key
...
For the list of region keys, see[Availability by Region](https://docs.oracle.com/en-us/iaas/Content/Registry/Concepts/registryprerequisites.htm#regional-availability). tenancy namespace
...
Note that although a repository name can include slash characters, the slash does not represent a hierarchical directory structure. It is simply one character in a string of characters. As a convenience, you might choose to start the names of several different repositories with the same string, perhaps ending in a slash (such as`project01/`). Such a string is sometimes called a 'repository name prefix'. But a repository named`project01/acme-web-app`need not have any relationship with a repository named`project01/my-test-app`. Using the same repository name prefix for some repositories simply makes it easier to organize and control access to them in Container Registry, which can contain many other repositories. registry identifier
...
- `us-phoenix-1.ocir.io/cbujx0t3wa3r`image path
...
- `us-phoenix-1.ocir.io/cbujx0t3wa3r/my-hello-app:latest`image URI

An image URI (Uniform Resource Identifier) identifies a particular image in a registry, and includes a version identifier, or an image digest, or both. An image URI has one of the following formats:
- `<tenancy-namespace>/<repo-name>:<version>`
- `<tenancy-namespace>/<repo-name>@<image-digest>`
- `<tenancy-namespace>/<repo-name>:<version>@<image-digest>`

For example:
- `ansh81vru1zp/project01/acme-web-app:v2.0.test`
- `ansh81vru1zp/project01/acme-web-app@sha256:abcd1234efgh5678...`
- `cbujx0t3wa3r/my-hello-app:latest@sha256:abcd1234efgh5678...`

Note that an image URI does not include a registry domain. version identifier
