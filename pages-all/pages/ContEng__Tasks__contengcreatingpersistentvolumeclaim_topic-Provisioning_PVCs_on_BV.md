...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengcreatingpersistentvolumeclaim_topic-Provisioning_PVCs_on_BV.htm
- Fetched: 2026-09-05 01:54 CDT

# Provisioning PVCs on the Block Volume Service

Find out how to provision persistent volume claims for clusters you've created using Kubernetes Engine (OKE) by attaching volumes from the Block Volume service.
...
The upstream Kubernetes project is deprecating the FlexVolume volume plugin in Kubernetes version 1.23. Removal of the feature will adhere to guidelines in the[Kubernetes Deprecation Policy](https://kubernetes.io/docs/reference/using-api/deprecation-policy/).
...
- Specify that`oci`is not the default storage class by entering:

```

```

- Specify that`oci-bv`is the default storage class by entering:
...
When you create a dynamically provisioned volume snapshot, Kubernetes Engine creates a`VolumeSnapshotContent`object. Do not modify the`VolumeSnapshotContent`objects that Kubernetes Engine creates, nor create your own`VolumeSnapshotContent`objects, when creating dynamically provisioned volume snapshots.
...
To statically provision a volume snapshot from an existing block volume backup, you first create the block volume backup (see[Creating a Manual Backup for a Block Volume](https://docs.oracle.com/iaas/Content/Block/Tasks/create-bv-backup.htm)).
...
To offer Ultra High Performance characteristics, Ultra High Performance block volumes must be attached to compute instances hosting worker nodes using a multipath-enabled attachment.

To create a PVC backed by a block volume with an Ultra High Performance level:
- Follow the instructions to create a node pool (see[Creating a Managed Node Pool](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/create-node-pool.htm)), and specify the following:
- Specify a Bare Metal (BM) or Virtual Machine (VM) shape that is both supported by Kubernetes Engine and that also supports the Ultra High Performance level.

Note that Ultra High Performance block volumes require shapes that support multipath-enabled attachments.

See:
- [Supported Shapes for Managed Nodes and Virtual Nodes](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/../Reference/contengimagesshapes.htm#shapes)for the shapes supported by Kubernetes Engine
...
- Specify a label to add to all worker nodes in the node pool to indicate that the worker nodes support the Ultra High Performance level. For example,`uhp: supported`
- If you want to attach an Ultra High Performance block volume to the compute instances hosting worker nodes in the node pool as an iSCSI attachment, install and enable the Block Volume Management plugin on the instances.
...
- About writing custom cloud-init scripts, see[Using Custom Cloud-init Initialization Scripts to Set Up Managed Nodes](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengusingcustomcloudinitscripts.htm).
- Create a storage class definition for block volumes with an Ultra High Performance performance level, and set`vpusPerGB`in the storage class definition to a value between 30 and 120.
...
Create a PVC provisioned by the storage class you just created, and include a request for storage, as follows:
- 
...
Note that raw block volumes are not supported for Ultra High Performance block volumes.

### Setting Raw Block Volume accessModes to ReadWriteOnce or ReadWriteMany

When you want a raw block volume to be exclusively mounted by a single pod at any given time (for example, when concurrent access to the same volume by multiple pods could lead to data corruption or inconsistencies), then set`accessModes`as`ReadWriteOnce`in the PVC manifest. The`ReadWriteOnce`mode is the most common and default access mode.

When you want a raw block volume to be accessible by multiple pods simultaneously (for example, when multiple pods running on different nodes need concurrent access to the same shared storage), then set`accessModes`as`ReadWriteMany`in the PVC manifest. Note that it is your responsibility to put in place appropriate access controls and permissions to ensure data integrity when you specify`ReadWriteMany`as the access mode.

Note the following:
- You cannot update the access mode of an existing PVC from`ReadWriteOnce`to`ReadWriteMany`. Similarly, you cannot update the access mode of an existing PVC from`ReadWriteMany`to`ReadWriteOnce`.
- You can only specify`ReadWriteMany`
