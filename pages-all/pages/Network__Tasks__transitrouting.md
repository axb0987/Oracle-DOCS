...
- The VCN that acts as the hub uses a[dynamic routing gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingDRGs.htm)(DRG) to communicate with the on-premises network. This hub VCN peers with each VCN acting as a spoke (referred to as spoke VCNs in this topic). The hub and spoke VCNs use[local peering gateways](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/localVCNpeering.htm)(LPGs) to communicate.
...
One of the VCNs acts as the hub (VCN-H) and connects to the on-premises network by way of[FastConnect](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Concepts/fastconnect.htm)or an[Site-to-Site VPN](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingIPsec.htm). The other VCNs are[locally peered with the hub VCN](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/VCNpeering.htm). The traffic between the on-premises network and the peered VCNs transits through the hub VCN. The VCNs must be in the same region but can be in different tenancies.

### Gateways Involved in Transit Routing

The next diagram shows the gateways on the VCNs. The hub VCN has a[Dynamic Routing Gateway (DRG)](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingDRGs.htm), which is the communication path with the on-premises network. Each locally peered spoke VCN uses a pair of[local peering gateways (LPGs)](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/localVCNpeering.htm)that anchor the peering connection: one LPG is on the hub VCN and the other is on the spoke VCN.
[
...
Callout 3: VCN route table associated with DRG attachment
Destination CIDR Route Target
192.168.0.0/16 LPG-H-1
...
- Traffic leaves the on-premises network and reaches the DRG. The traffic's destination is in Subnet-1 (for example, 192.168.0.5).
- 

The DRG attachment's associated route table has a rule for 192.168.0.0/16. It matches the destination and sends the traffic to the route target:
...
- LPG-H-1 receives the traffic.
- Egress traffic leaving a VCN through an LPG is automatically routed to the LPG's peered LPG, which is LPG-1 in this situation. That routing occurs automatically because of the peering connection between the two LPGs.
- LPG-1 receives the traffic.
...
- Traffic comes from an instance in Subnet-1 in the spoke VCN. The traffic's destination is in the on-premises network (for example, 172.16.0.3).
- Subnet-1's associated route table has a rule for 172.16.0.0/12. It matches the destination and sends the traffic to the route target, LPG-1.
- LPG-1 receives the traffic.
- Egress traffic leaving a VCN through an LPG is automatically routed to the LPG's peered LPG, which is LPG-H-1 in this situation. That routing occurs automatically because of the peering connection between the two LPGs.
- LPG-H-1 receives the traffic.
- 

LPG-H-1's associated route table has a rule for 172.16.0.0/12. It matches the destination and sends the traffic to the route target:
...
- Egress traffic leaving the VCN through the DRG is routed based on Site-to-Site VPN and FastConnect configuration. No explicit rules in the DRG attachment's route table are required.
...
- Traffic comes from an instance in Subnet-1 in the spoke VCN. The traffic's destination is in a subnet in the hub VCN (for example, 10.0.0.3).
- Subnet-1's associated route table has a rule for 10.0.0.0/16. It matches the destination and sends the traffic to the route target, LPG-1.
- LPG-1 receives the traffic.
- Egress traffic leaving a VCN through an LPG is automatically routed to the LPG's peered LPG, which is LPG-H-1 in this situation. That routing occurs automatically because of the peering connection between the two LPGs.
- LPG-H-1 receives the traffic.
- Traffic coming in to a VCN through an LPG and destined for an address in the VCN is automatically routed to the destination by VCN local routing. No explicit route rules are required.
...
If you set up transit routing through a private IP in the hub VCN, remember that the LPG-H-1 route table only controls routing of traffic destined for addresses outside the hub VCN . Traffic destined for addresses within the VCN is handled by the hub VCN local routing, which takes precedence and always routes the traffic directly to the packet's destination address. This means that you can't route traffic destined for addresses inside the hub VCN through the private IP being used for the transit traffic through the hub. Even if the LPG-H-1 route rule uses a destination = 0.0.0.0/0 and target = 10.0.8.3, the hub VCN local routing takes precedence and routes the traffic directly to the destination in the hub VCN instead of the private IP.
...
- [FastConnect](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Concepts/fastconnect.htm)
- [Site-to-Site VPN](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Tasks/managingIPsec.htm)
- [Dynamic Routing Gateways](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Tasks/managingDRGs.htm)
...
When setting up local peering between two VCNs, be sure to[establish the connection between the LPGs](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Tasks/localVCNpeering.htm#establish_cxn). It can be easy to overlook that part of the process. Notice that you do not create the route table associated with the LPG on the hub VCN (LPG-H-1) yet. That comes in a later step.For more information and instructions:
- [Setting Up a Local Peering](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Tasks/localVCNpeering.htm#Setting)
...
- For the spoke VCN, view the list of subnets.
- For the subnet of interest, look at its details and click the link for its associated route table.
- 

Edit the route table to include a rule that sends traffic to the on-premises network:
- Click Add Route Rules .
- 

Enter this information for the route rule:
...
- In the Console, view the hub VCN's details.
- Under Resources , click Route Tables to view the VCN's route tables.
...
Click + Additional Route Rule , and enter this information for the route rule:
- Target Type: Local Peering Gateway.
...
Associate the route table (called DRG Route Table in this example) with the hub VCN's DRG attachment:
- While still viewing the hub VCN's details, click Dynamic Routing Gateways to view the attached DRG.
- Click the Actions menu (three dots) , and then click Associate Route Table .
...
Create a route table for the hub VCN's LPG for this spoke:
- While still viewing the hub VCN's details, click Route Tables .
- Click Create Route Table .
- 
...
Associate the route table (called Hub LPG-# Route Table in this example) with the hub VCN's LPG for the spoke of interest:
- While still viewing the hub VCN's details, click Local Peering Gateways to view the hub VCN's LPG for this spoke.
- For the LPG you're interested in, click the Actions menu (three dots) , and then click Associate With Route Table .
- 
...
- Repeat Tasks 3 through 5 for the new spoke VCN.
- 
...
In this task, you set up the hub VCN. The hub VCN must have two subnets: one for the frontend VNIC on the instance, and one for the backend VNIC on the instance. Oracle recommends using regional private subnets, unless there are resources in the frontend subnet that need internet access.
...
- If you haven't already, create the instance in the hub VCN. See[Creating an Instance](https://docs.oracle.com/iaas/Content/Compute/Tasks/launchinginstance.htm). The primary VNIC is created in the subnet you specify.
- Create a secondary VNIC for the other subnet and configure the OS to use it. See[Managing VNICs](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingVNICs.htm#console).
- Disable the source/destination check on each of the VNICs. See[Overview of VNICs and Physical NICs](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingVNICs.htm#overview).
...
- For each of the private IPs you created, record the private IP address (for example: 10.0.4.3).
- Configure the instance as necessary for the job it performs (for example, configure the firewall or intrusion detection system on the instance).
...
Associate the route table (called DRG Route Table in this example) with the hub VCN's DRG attachment:
- While still viewing the hub VCN's details, click Dynamic Routing Gateways to view the attached DRG.
- Click the Actions menu (three dots) , and then click Associate With Route Table .
- 
...
- For the spoke VCN, view the list of subnets.
- For the frontend subnet, look at its details and click the link for its associated route table.
- 

Edit the frontend subnet's route table to include a rule that sends traffic destined for the spoke VCN to the private IP in the frontend subnet:
- Click Add Route Rules .
- 
...
- Description: An optional description of the rule.
- Click Add Route Rules .
- For the backend subnet, look at its details and click the link for its associated route table.
- 

Edit the backend subnet's route table to include a rule that sends traffic destined for the on-premises network to the private IP in the backend subnet:
- Click Add Route Rules .
- 
...
Repeat task 7 with these changes:
