...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengaddingserviceaccttoken.htm
- Fetched: 2026-09-05 01:54 CDT

# Adding a Service Account Authentication Token to a Kubeconfig File

Find out how to add a service account authentication token to the kubeconfig file of a Kubernetes cluster you've created using Kubernetes Engine (OKE).
...
Note that to run the commands in this topic, you must have the appropriate permissions. See[About Access Control and Kubernetes Engine (OKE)](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/../Concepts/contengaboutaccesscontrol.htm).
...
- Obtain an authentication token for the kubeconfig-sa service account as follows:
- In a text editor, create a file (for example, called oke-kubeconfig-sa-token.yaml) to create a secret (for example, named oke-kubeconfig-sa-token) with the following content:
...
For example, if you have previously deployed a sample Nginx application on the cluster (see[Deploying a Sample Nginx App on a Cluster Using Kubectl](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengdeployingsamplenginx.htm)), enter the following command:
