...
- User logs in to the OIDC identity provider to obtain an id_token.
- User issues a kubectl command to the cluster, passing the id_token as an argument.
- The Kubernetes API server running on the cluster validates the id_token using information from the OIDC identity provider.
- The Kubernetes API server performs Kubernetes RBAC authorization.
- If the user is authorized, the Kubernetes API server performs the requested operation, and kubectl returns a response to the user.
...
- Use the kubernetes-apiserver log to troubleshoot issues with initiating the OIDC authenticator plugin, and issues with token validation. For more information, see[Viewing Kubernetes Engine (OKE) Service Logs](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengviewingservicelogs.htm).
...
Stateful Identity provider's IP address Identity provider-dependent

Allow traffic to external OIDC identity provider.
...
- Using your preferred JSON editor, create a new file in JSON format to update the cluster's external OIDC identity provider configuration, and include at least the following parameters required for the configuration:
...
- Save the JSON file containing the external OpenID Connect (OIDC) identity provider configuration, with a name and in a location of your choice. For example, as`/users/jdoe/update-oidc-config`
- Use the`oci ce cluster update`command to update the existing cluster:
```
...
- Convert the authentication configuration .yaml file to a Base64 encoded string.
- 

Use the`oci ce cluster create`command and include the following parameters:
- `--open-id-connect-auth-enabled`set to`true`
- `--oidc-configuration-file`set to the Base64 encoded string that you created from the authentication configuration file.
...
Note that having set the`--oidc-configuration-file`parameter, do not use other parameters of the`oci ce cluster create`command to configure OIDC authentication. Only use fields in the authentication configuration file to control OIDC authentication.
...
Use the[CreateCluster](https://docs.oracle.com/iaas/api/#/en/containerengine/latest/Cluster/CreateCluster)operation to create the cluster, and specify values for the following attributes of the`openIdConnectTokenAuthenticationConfig`property:
- `isOpenIdConnectAuthEnabled`set to`true`
...
- Log in to the external OIDC identity provider.
- Make a note of the`id_token`and the`refresh_token`that the OIDC identity provider provides.
- Set up a kubeconfig file to access the OIDC-enabled cluster by following the instructions in[Setting Up Cloud Shell Access to Clusters](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengdownloadkubeconfigfile.htm#cloudshelldownload)or[Setting Up Local Access to Clusters](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengdownloadkubeconfigfile.htm#localdownload)as appropriate, depending on how you want to access the cluster.
- Define a user in the kubeconfig file that is to be authenticated with the external OIDC identity provider by running the following command:
...
- Define a context in the kubeconfig file for the new user that is to be authenticated using the external OIDC identity provider, by running the following command:
```
