...
- Source: https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/create-job-lock-file.htm
...
If the stack is older, then it must be upgraded to use Terraform Registry. See[Using Terraform Registry with an Older Stack](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/update-stack-tf-reg.htm).
...
These steps show how to retrieve the latest providers for a job in a stack. You can also retrieve the latest providers for a job[in a compartment](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/list-jobs.htm).
...
- In the panel for the job type that you selected, select Show advanced options and select Upgrade provider versions .
...
For example, to retrieve the latest providers when running a plan job, use the`[](https://docs.oracle.com/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/resource-manager/job/create-plan-job.html)oci resource-manager job create-plan-job`command to run a plan job, with`--job-operation-details-is-provider-upgrade-required`set to`true`.
