...
- Source: https://docs.oracle.com/en-us/iaas/Content/Compute/Concepts/dedicatedvmhosts_topic-infrastructure-maintenance.htm
...
To avoid disruption to your workloads, you must migrate all affected instances using[reboot migration](https://docs.oracle.com/en-us/iaas/Content/Compute/Concepts/dedicatedvmhosts_topic-infrastructure-maintenance.htm#top__reboot-migration)or[manual migration](https://docs.oracle.com/en-us/iaas/Content/Compute/Concepts/dedicatedvmhosts_manual-migration.htm)to another dedicated virtual machine host before the scheduled time.

## Migrating during Unplanned Maintenance

When the underlying infrastructure for a dedicated virtual machine host fails because of software or hardware issues, you must migrate all affected instances to another dedicated virtual machine host as soon as possible using[reboot migration](https://docs.oracle.com/en-us/iaas/Content/Compute/Concepts/dedicatedvmhosts_topic-infrastructure-maintenance.htm#top__reboot-migration)or[manual migration](https://docs.oracle.com/en-us/iaas/Content/Compute/Concepts/dedicatedvmhosts_manual-migration.htm).
...
- Select the source dedicated virtual machine host. Note the Action Required message for the host.
- Select Migrate instances .
- Select the destination dedicated virtual machine host using the host list provided or by specifying the host OCID.
- Select Migrate .
Important  
...
To monitor the migration process, select Work Requests . A job is created for each instance. The progress for each instance migration is shown in the console.
