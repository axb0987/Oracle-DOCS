...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/federatingADFS.htm
...
Note all the Active Directory groups that you want to map to Oracle Cloud Infrastructure IAM groups. You will need to enter these in the Console in the next step.
...
Copy the URL. The URL looks similar to:
...
Add Oracle Cloud Infrastructure as a trusted relying party :
- 

From the AD FS Management Console, right-click AD FS and select Add Relying Party Trust .
- 
...
- Select Browse and select the metadata.xml file that you saved.
- Select Next .
- 

Set the display name for the relying party (for example, Oracle Cloud Infrastructure) and then select Next .
...
- Check Open the Edit Claim Rules dialog for this relying part trust when the wizard closes and then select Close .
...
- In the Add Transform Claim Rule Wizard , select Transform an Incoming Claim , and select Next .
- 
...
- Select Finish .
- The rule is displayed in the rules list. Select Add Rule .
...
Add the groups rule:
- Under Claim rule template, select Send Claims Using a Custom Rule . Select Next .
- 
...
Custom rule: Enter the following custom rule:
...
Custom rule: Enter an appropriate filter rule. For example to send only groups that begin with the string "OCI", enter the following:
...
Give federated users the URL for the Oracle Cloud Infrastructure Console,[https://cloud.oracle.com](https://cloud.oracle.com), and the name of your tenant. They'll be prompted to provide the tenant name when they sign in to the Console.
...
Select Add Mappings .
- Under Identity Provider Group , enter the Active Directory group name. The name you enter here must match exactly the name in Active Directory.
- 
