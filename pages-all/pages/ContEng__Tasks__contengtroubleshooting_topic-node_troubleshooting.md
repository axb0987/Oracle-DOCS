...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengtroubleshooting_topic-node_troubleshooting.htm
...
- Select the Management tab of the instance details page.
- In the Run command section, find the command in the list, and then select View Command Details from the Actions menu (three dots) beside the command.
...
For detailed instructions to establish an SSH connection, see[Connecting to Managed Nodes Using SSH](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengconnectingworkernodesusingssh.htm). At a high level, the steps are:
- Find out the IP address of the worker node instance that you want to troubleshoot, and make a note of it.
...
For example, on the Node pools tab of the cluster details page, select the node pool containing the worker node. Select the Nodes tab, and then select the name of the node you are interested in to display the instance details page. Select the Networking tab to see the instance's IP address.
- In a terminal window, enter`ssh opc@<node_ip_address>`to connect to the worker node, where`<node_ip_address>`is the IP address of the worker node that you made a note of earlier. For example, you might enter:
```
