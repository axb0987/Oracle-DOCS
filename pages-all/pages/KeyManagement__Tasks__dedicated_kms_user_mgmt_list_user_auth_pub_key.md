...
- Source: https://docs.oracle.com/en-us/iaas/Content/KeyManagement/Tasks/dedicated_kms_user_mgmt_list_user_auth_pub_key.htm
- Fetched: 2026-09-05 02:33 CDT

# Listing User Authentication Public Key

Command to list public key.
...
The`listUserAuthPubKeys`returns a list of registered authorization keys, their key index and user ID with which the key is associated.

In the User Management utility, open a command prompt and run`listUserAuthPubKeys`command to return a list of registered authorization keys, their key index and user ID with which the key is associated.
