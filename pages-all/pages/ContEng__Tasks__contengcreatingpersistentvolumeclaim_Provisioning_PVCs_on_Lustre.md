...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengcreatingpersistentvolumeclaim_Provisioning_PVCs_on_Lustre.htm
...
- Create a manifest file (for example, in a file named lustre-dyn-st-class.yaml), specify a name for the new storage class, and specify values for required and optional parameters:
```
...
- Create a file system in the File Storage with Lustre service, selecting the Encrypt using Oracle-managed keys encryption option. See[Creating a Lustre File System](https://docs.oracle.com/iaas/Content/lustre/file-system-create.htm).
- Create security rules in either a network security group (recommended) or a security list for both the Lustre file system, and for the cluster's worker nodes subnet.
The security rules to create depend on the relative network locations of the Lustre file system and the worker nodes which act as the client, according to the following scenarios:
- [Option 1: Client and Lustre in Different Subnets](https://docs.oracle.com/iaas/Content/lustre/security-rules.htm#top__different-subnet)
- [Option 2: Client and Lustre in the Same Subnet](https://docs.oracle.com/iaas/Content/lustre/security-rules.htm#top__same-subnet)
...
Verify that the PV has been created successfully by entering:
```
...
Verify that the PVC has been created and bound to the PV successfully by entering:
```
...
- In the PV manifest described in[Provisioning a PVC on an Existing Lustre File System](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengcreatingpersistentvolumeclaim_Provisioning_PVCs_on_Lustre.htm#contengcreatingpersistentvolumeclaim_topic_Provisioning_PVCs_on_Lustre_Existing_file_system), add the`spec.mountOptions`field, which enables you to specify how the PV should be mounted by pods.

For example, in the lustre-pv-example.yaml manifest file shown in[Provisioning a PVC on an Existing Lustre File System](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengcreatingpersistentvolumeclaim_Provisioning_PVCs_on_Lustre.htm#contengcreatingpersistentvolumeclaim_topic_Provisioning_PVCs_on_Lustre_Existing_file_system), you can include the`mountOptions`field as follows:
```
