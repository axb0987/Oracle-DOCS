# Configure Integration Between Oracle Access Governance and Oracle Cloud Enterprise Performance Management
- Source: https://docs.oracle.com/en-us/iaas/Content/access-governance/oracle-epm-configure-integration-epm.htm
...
- Choose a compartment where your Oracle Cloud Enterprise Performance Management service instance is located, and then select the domain.
- Select the User management tab.
- Click Create .
- In the First name and Last name fields enter the user's name.
- To have the user sign in with their email address, enable the Use the email address as the username check box and enter the email address for the user account.
- To have the user sign in with their user name, clear the Use the email address as the username check box and enter the user name for the user.

### Assign Identity Administrator to the Service User

Assign Identity Domain Administrator role to the service user.

- On the Domains page, select the Administrators tab.
- In the Identity Domain Administrator section, click Add users .
- Search the service user.
- Select the service user check box and then click Add Users .
...
- Select the EPM Cloud service instance.
- Select the Application roles tab.
- Locate and select the Service Administrator role.
- For the Service Administrator role, select the Actions icon, and select Manage users .
- Select Assign users .
- Select the service user created in the previous step, and then click Assign .
...
To create a certificate, refer the steps as explained in[Creating a Certificate in OCI IAM](https://docs.oracle.com/iaas/Content/certificates/creating-certificate.htm).
...
- Enter the same alias name that you provided while generating the keystore file certificate alias, and import the`.cer`file
- Click Import .
...
- Select JWT assertion and Refresh token grant types:
...
- Select Oracle Cloud Enterprise Performance Management .
- Click Next .
...
- Read and select the prerequisite check box: I have an active Oracle Cloud Infrastructure orchestrated system already , indicating that you must have an existing active connection with Oracle Cloud Infrastructure for your service instance.
- Click Next .
...
- On the Integration settings step of the workflow, enter the details required to allow Oracle Access Governance to connect to your Oracle Cloud Enterprise Performance Management system.

Integration settings
Parameter Name Description

EPM Host Name URL to access your Oracle Cloud Enterprise Performance Management system. For example:
```

```

OCI domain name Enter the OCI IAM Domain name where the integrated application is created.
Domain URL Enter the OCI IAM Domain URL. To find your Domain URL, see[Finding an Identity Domain URL](https://docs.oracle.com/iaas/Content/Identity/api-getstarted/locate-identity-domain-url.htm).
...
Private Key Enter the encrypted private key (`.pem`).

Certificate Alias Enter the certificate alias configured while generating a certificate.

What is the username? EPM username

What is the EPM application resource scope? The Scope field is mandatory for EPM version 26.07 and later. For earlier versions, the default scope (urn:opc:idm:__myscopes__) can be used. Example scope:
```
