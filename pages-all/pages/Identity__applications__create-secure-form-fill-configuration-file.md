...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/applications/create-secure-form-fill-configuration-file.htm
...
Install the Secure Form Fill Admin Client. See[Install the Secure Form Fill Admin Client](https://docs.oracle.com/en-us/iaas/Content/Identity/applications/install-secure-form-fill-admin-client.htm).
...
- Launch the Secure Form Fill Admin Client.
- Right-click Applications , and then choose New Web App . Alternatively, use any of the following options:
...
- On the Add Application dialog, enter the name of the application.

Important  
  
The application name must be the same name that you use when you create the secure form fill application in IAM.
- Leave all other default options selected, and then select Finish .
- Choose the Logon form type. This form is used to set up your Web app template.

Note  
  
No other form type is supported.
- In the Address field, enter the URL for the Web app, select Go , and then navigate to the login page.
...
- Select OK .
- On the Web dialog, make any of the following changes as necessary.

Option Description

Identification Tab

Select Edit to make changes. This is the URL or the URLs of the form to configure. For applications that have varying text in their URLs, you can use substrings or regular expressions to specify how to match the variable text.
...
Select a field, and then using the Edit button, adjust the primary form fields used for detection and injection of the user name and password fields as well as the submit button.
...
- Select OK .
- Use the following tabs to make any necessary changes:
...
- To export the file, select File , Export , select the application to export, and then select OK .
- Name the file.
