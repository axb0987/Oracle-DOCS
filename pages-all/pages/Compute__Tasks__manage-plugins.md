# Oracle Cloud Agent
- Source: https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins.htm
...
Oracle Cloud Agent: Oracle Cloud Agent is supported on current[platform images](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/../References/images.htm)and on[custom images](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/../References/bringyourownimage.htm)that are based on current platform images. Oracle Cloud Agent is installed by default on current platform images.
...
`Fleet Application Management`Manages and monitors executions of discovery and patching for applications managed by Fleet Application Management. See[Fleet Application Management](https://docs.oracle.com/iaas/Content/fleet-management/home.htm).
`High Performance Computing`Performs complex calculations and processes data faster than traditional Compute. See[High Performance Computing](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/../References/high-performance-compute.htm).
...
`Vulnerability Scanning`Scans the instance for potential security vulnerabilities like OS packages that require updates. See[Scanning Overview](https://docs.oracle.com/iaas/Content/scanning/using/overview.htm).
...
If Oracle Cloud Agent isn't installed, or if the installed version is not the[latest version](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins.htm#Oracle_Cloud_Agent_software_versions), install the latest version by running the following command:
...
- To obtain the Oracle Cloud Agent installation file,[contact support](https://docs.oracle.com/iaas/Content/GSG/Tasks/contactingsupport.htm).
...
Obtain the Oracle Cloud Agent installation file by[contacting support](https://docs.oracle.com/iaas/Content/GSG/Tasks/contactingsupport.htm).
- Follow the steps to[create an instance](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/launchinginstance.htm), until the advanced options.
...
For legacy versions of Windows images, ensure that cloudbase-init is supported. See[WinRM and cloudbase-init on Windows images](https://docs.oracle.com/iaas/releasenotes/changes/595afbb7-de0c-4934-8074-5b1ed6be1b56/).
- Click Create .
...
- If you enabled a plugin, if necessary, perform any configuration tasks that are required before you can use the plugin. For information about how to configure each plugin, see the documentation for each plugin in[Available Plugins](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins.htm#available-plugins).
...
[Connect to the instance](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/accessinginstance.htm)and then do one of the following things:
- 
...
[To disable automatic updates from Oracle Cloud Agent Updater](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins.htm#)
...
This configuration prevents the agent from checking for updates. The setting persists even after performing manual upgrades.

Restarting the Updater Service
After applying this configuration, restart the updater service to apply the changes.
- 
For Oracle Linux, run the following command:
```
...
Oracle Cloud Agent:
- Added support for enabling or disabling Falcon plugin deployment with host-level granularity.

High Performance Computing:
- Fix hugepages logic for multiplanar shapes.
- Added Open vSwitch health checks and automatic restoration of volatile flows and groups after ovs-vswitchd restarts on`BM.GPU.B300.8`,`BM.GPU.B300.HS.8`, and`BM.GPU.GB300.4`multiplanar hosts.
- Preserved VF global IPv6 connectivity while removing duplicate addresses during multiplanar configuration.
- Added RDMA configuration metadata for`BM.GPU.RTXPRO.8`and improved repeat configuration on Ubuntu 24 by preparing RDMA network directories only once and safely handling previously moved directories.
- Disabled PCIe Access Control Services on`BM.GPU.RTXPRO.8`as required by its RDMA configuration.
- Added configuration options to skip all Pollara post-configuration scripts or individually skip the QoS, DCQCN, and Access Control Services scripts.
- Made RDMA policy-rule configuration idempotent by continuing when the priority 0 rule is already absent, while still failing when the rule cannot be removed.
- Added`arp_evict_nocarrier`support to RDMA ARP settings and safely skips this optional setting on kernels that do not provide it.
- Added`BM.GPU.H200-LM.8`shape support, including RDMA metadata and optional congestion-control configuration.
- Waits for RDMA authentication to complete before reporting RDMA configuration as complete, and reports a failure if authentication does not become ready.
- Expanded HPC plugin availability across OCI realms for supported operating systems and shapes.
- Validates PCIe fault events before ingestion and skips malformed events that are missing the event type, AER error code, or PCI address.
- Added link-up event collection for Mellanox and AMD Ionic devices so recovery transitions are included in FMS host-event payloads.
- Added FEC histogram collection for supported mlxlink versions, with an automatic fallback when histogram options are unavailable.
- Enriched AMD GPU and host-event payloads with stable GPU UUIDs when available.
- Improved first-boot multiplanar reliability by waiting for SLAAC on every VF before duplicate-address cleanup and verifying that each VF finishes with exactly one global IPv6 address.
1.61.0 June 05, 2026

Compute Instance Run Command:
- Added support for Ubuntu snap packages.

Bastions:
- Fixed an insecure transport cipher suite downgrade vulnerability.
- Hardened file creation and ownership changes to prevent symlink traversal and path race conditions.

High Performance Computing:
- Added stable IPv6 addresses for multiplanar hosts.
- Detects RDMA device renaming inconsistencies earlier and fails fast.
- Hardened Pollara scripts to handle RDMA network device names consistently.
- Added PCIe CE and UE aggregation to FMS event payloads.
- Added NVLink information to FMS payloads.
- Added advanced RDMA NIC metrics.
- Added`BM.GPU.B300.HS.8`multiplanar support.
- Daemonized OVS under systemd for multiplanar hosts and added health checks.
- Flushes duplicate global IPv6 addresses on multiplanar VFs.

OS Management Hub:
- Added DualStack endpoint and IPv6 SingleStack support.

Cloud Guard Workload Protection:
- Removed obsolete WLP install handling and made warning-only RPM`.rpmsave`output non-fatal during install and uninstall.

Fleet Application Management Service:
- Added DualStack endpoint and IPv6 SingleStack support with IPv6-capable executor handling.
- Addressed CVE-2025-58189, CVE-2025-58187, CVE-2025-58188, CVE-2025-58185, and CVE-2025-61723, and deferred Resource Principal authentication until the service client is needed.

Oracle Cloud Agent:
- Corrected metadata IP detection for proxy and IMDS whitelist environments.
1.60.0 May 07, 2026

Oracle Cloud Agent Updater:
- Fixed updater log rotation when`updater.log`is missing or full.

Compute Instance Run Command:
- Improved cleanup of deployment working directories after command completion.

Cloud Guard Workload Protection:
- Added signature verification for Instance Security agent package updates.

High Performance Computing:
- Added new event types and metadata for OHMP FMS transactions.
- Added GB300 multiplanar support for Ubuntu.
- Added bounded RDMA bad-state recovery for wpa_supplicant authorization issues.
- Fixed Pollara (`BM.GPU.MI355X.8`) device name ordering so mappings persist after card reset.
- Increased wait time for SLAAC IP assignment before policy-based routing is triggered.
- Added PCIe fault pipeline updates.

Management Agent:
- Added support for IPv6-capable Oracle Management Agent. Full support will be available when all dependencies are available approximately June, 2026.

Java Management Service:
- Addressed CVE-2024-7254, CVE-2025-58057, CVE-2025-67735, CVE-2026-33870, CVE-2025-55163, and CVE-2026-33871.
- Improved Java library scanning for JAR files that do not include`pom.xml`.
- Fixed Java attachment filtering when Java Home paths are excluded.
- Fixed a startup race condition when the plugin receives a stop request during startup.
1.59.0 April 03, 2026

Oracle Cloud Agent:
- CVE-2025-11579/CVE-2025-15558/CVE-2026-33186 vulnerability fixes.
- Added support for IPv6 SingleStack instances (agent and updater not all plugins). Full support will be available when all dependencies are available: approximately June, 2026.

Runcommand:
- Added support for IPv6 SingleStack instances. Full support will be available when all dependencies are available approximately June, 2026.

Compute Instance Monitoring:
- Added support for IPv6 SingleStack instances. Full support will be available when all dependencies are available approximately June, 2026.

Cloud Guard Workload Protection:
- Added the`wlp-doctor`script to the plugin for customer ease of access.

Management Agent:
- Enabled FIPS setting for realm`oc32`.

High Performance Computing:
- Using native kernel support for SLAAC instead of`radvd`.
- Added a new OHMP FMS pipeline in`oci-hpc-monitoring`(`oci-fms-events`) to collect, convert, and send host/RDMA/GPU hardware events to FMS.
- Lowered threshold for AMD RDMA Bitrate Error Fault.
- Execute Pollara (`BM.GPU.MI355X.8`) pre/post oca scripts from local embedded fs.
- Fixed missing`L40S-NC`shape in`rdma_settings`since OCA 1.48.0.
- Added support for IPv6 SingleStack instances. Full support will be available when all dependencies are available approximately June, 2026.
...
- Fixed Crypto and Performance Analysis completion issues in Containers.
- Fixed a Security finding in okhttp dependency.
- Improved application naming when application is launched from a`.class`file instead of a`.jar`file.
- Made improvements in usage tracker log processing.
- Fixed Java EPP listing issue by improving Java perf version detection logic.

Fleet Application Management Service:
- Limited`stdout`content to the most recent 4kb to prevent buggy behavior on larger output.

High Performance Computing:
- Change log level to warn when getTableNum returns error when a route table is not present.
- Added support for optionally enabling Round-Trip Time Congestion Control (RTTCC) on the following high-performance computing (HPC) shapes via updates to the`shapes.json`configuration:`BM.GPU.H200.8`,`BM.GPU.H200-NC.8`,`BM.GPU.B200.8`, and`BM.GPU.MI300X.8`. This allows tailored network optimization without affecting existing setups.
- IPv6 Policy based routing for`GB200 V3.4`.
- Support IPv6 fabric policy based routing table configurable via`rdma_features_enable.json`.
- Support B300 shape in Ubuntu.
- Support`MI355X.8-Pollara`shape.

Block Volume Management:
- Make iSCSI Logins parallel.
- Reboot Migration metadata protection. Reboot Migrations for VM onto the same Hypervisor left metadata from the old VM due to soft fail on delete volume attachment metadata. The change makes BVM plugin resilient by picking the most recent attachment version of metadata.
- Stabilize Device Path lookup.
- Improve the IMDS poll interval to 30s.
- Fix`iscsi`empty nodes by deleting them if an issue is encountered when adding them.
1.57.0 February 05, 2026

High Performance Computing:
- For shapes`BM.GPU.GB200-v3.4`,`BM.GPU.GB300.4`set`roce_adp_retrans_en`to 1 by default in`shapes.json`.
- IPv6 fabric policy based routing with feature flag enabled.
- Support for`BM.GPU.B300.8`.
- Correct RDMA NIC authentication retry behavior to handle transient authorization under high-latency dependencies and add regression unit test coverage.

Oracle Java Management Service:
- Updated`jmigrate`version to`0.0.69`.
1.56.0 January 15, 2026

Compute Instance Run Command:
- Fixed flat-file decompression for archived directories.

Compute Instance Monitoring:
- Fixed spurious 100% memory utilization readings on Oracle Linux 9.6.

High Performance Computing:
- Added logic to skip running mstfwreset for GB300, GB200-v3.4, and MI355X-v1.8 shapes.
- Detects and reports XID 149 occurrences as faults for GB200 shapes.
- Added dual output format support for amd-smi.
- In IPv6 fabric environments, enabled state file handling for oci-hpc-rdma-configure and oci-hpc-mlx-configure.
- Added support for MI355X v1.8.
- Improved monitoring coverage for GB200.
- Enabled IPv4 addressing selection based on network topology.
- Updated Shapes.json with GB200 v3.4 and GB300. Also improved IPv6 fabric detection.

Oracle Java Management Service:
- Addressed security advisory GHSA-jfcv-jv9g-2vx2.
1.55.0 November 06, 2025

Oracle Cloud Agent:
- Ensures the in‑memory agent configuration is refreshed after upgrade so the latest configuration is used immediately (no manual restart needed).

Custom Logs Monitoring:
- Enablement on Oracle Linux 10.

Fleet Application Management Service:
- Enablement in additional OC1 regions; general stability and reliability improvements.

High Performance Computing:
- Move RDMA network files for Ubuntu 24.
- Recover RDMA routes following link DOWN and UP events.
- Fix support for GB200-v2.4 SMC shapes.
- Fix ping between RDMA interfaces.
- Improve RDMA link fault detection.
- Update log retention to 7 days.
- Support IPv6 in OCA HPC using SLAAC.

Management Agent:
- Oracle Linux 10 support for the Management Agent plugin; updated FIPS settings for supported realms.

Oracle Java Management Service:
- Expanded detection of Java runtimes and applications (including non‑Oracle OpenJDK), improved container detection, and support for JDK 25 migration analysis.
- Startup parameters updated to enable needed JVM statistics for enhanced usage detection.
- JMS enablement expanded to additional realms.

Vulnerability Scanning:
- Limits nested scan depth in Syft-based application scanning to improve performance and avoid excessive recursion.
1.54.0 November 8, 2025

Fleet Application Management Service:
- Resource Principal AuthN and AuthZ implementation.

High Performance Computing:
- Support for H200-NC and GB200-v2 shape.
- Reduce fault false positives by requiring that fault conditions persist for at least 10 minutes.

Oracle Cloud Agent Updater:
- Add a check for dnf lock files during upgrades.

Oracle Java Management Service:
- Unify java.vendor string for JDKs from Sun era.
- Jmigrate should produce diagnostics for javafx package removal on target JDK8.
- Provide a script to collect plugin logs in case of failure.

Vulnerability Scanning:
- Use the correct timestamp to update last file scan timestamp.
- Use`osinfo`to determine type of scan command to issue on the host.

WebLogic Management Service:
- Enable WLMS in all realms.
- Fix the issue where the service endpoint is calculated incorrectly.
1.53.0 June 09, 2025

High Performance Computing:
- Removes deprecated Image Metadata Service v1 fallbacks.
Fleet Application Management Service:
- Bug fixes to address the conflict of polling multi-status command requests.
- Timeout implementation of the task execution according to runbook timeout property.

Block Volume Management:
- Prevents iscsi session leaks by removing an attachment from persistent attachments only after all iscsi sessions are logged out.
- Rearranges the multipathd restart attempts to post reset and logout of attachments.

Oracle Java Management Service:
- Skips Java Flight Recorder capture if the capture window is over.
- The JMS log manager has been added to configuration. Bastions:
- Fix vulnerability CVE-2024-24790. OCI Vulnerability Scan:
- Security Updates and bug fixes.
1.52.0 May 07, 2025 Weblogic Management Service:
- Updates OCI SDK to 2.150.0 Java Management Service:
- Fixes error code`422`for cryptoanalysis for JFR captures and uploads
- Fixes crypto and performance analysis work request completion issue for JFR capture and upload OS Management Service Agent:
- Minimizes OS Management Service Agent downtime when you restart OCA
- Adds a periodic check that stops orphaned osmh-agent processes
- 

Removes OS Management Service Agent (OSMS) and Autonomous Linux (ALX) as they are no longer supported as of April 23, 2025 High Performance Computing:
- Upgrades cert-manager library to 1.12.14
- Handles new format of throttle_status for AMD ROCM 632/driver 6.10.5
- Adds support to fetch OCA version for Debian OS
1.51.0 April 07, 2025

Block Volume Management:
- Adds regex fix to exactly match multipath friendly names during block volume detaches
- Adds fix to check for multipath to be active before performing UHP volume attachments sign in
- Deletes symlinks for consistent device paths post Detach of UHP volume attachments

Cloud Guard Workload Protection:
- Adds support to install WLP agent on Ubuntu 24.04 Fleet Application Management:
- Introduced support for Fleet Application Management (FAMS) plugin, now available in select regions
- Support will be expanded to more regions over time

High Performance Computing:
- Adds support for GB200 and B200 shapes
- Adds fix to ensure IP addresses, rules, and routes are set in OL9 after the initial install of OCA
- Adds fix that delays cable state check until the RDMA auth state isn't STARTED

Oracle Java Management Service:
- Addresses CVE-2024-29857 by updating the BC library
- Adds support to run Java flight recorder for the applications running on Enterprise performance pack
- Adds capture of Network data if the configuration is enabled at fleet
- Fixes parsing`healthStatus`if provided or deserialized as List
- Fixes race condition when creating`usagtracker.properties`file
- Adds JDK 24 support for Java Migration Analysis OS Management Service Agent
- Improves reliability of the`osmh-agent`auto-upgrade process
1.50.0 March 11, 2025
...
- Adds fix to prevent delayed RDMA configuration after initial Oracle Cloud Agent installation in Oracle Linux 9 instances Cloud Guard Workload Protection:
...
- Updates shapes.json to latest version: 20250130-0.5.8 OS Management Service Agent
- Supports Boostrapper image downloads in ONSR
- Adds automatic upgrade for osmh-agent Cloud Guard Workload Protection:
...
- Releases version JMS 9.0 Cloud Guard Workload Protection:
- Fixes plugin for fatal logs Compute RDMA GPU Monitoring:
- Enables GPU fault metrics for MI300X
- Adds the ability to set GPU clocks within the OCA plugin by adding a new GPU Configuration module within the HPC Configure plugin Compute Instance Run Command:
- Run command reverts to main endpoint fix
1.43.0 July 11, 2024 Monthly patches and bug fixes.
...
- Updates shapes.json to allow ORAP support for MI300X, L40S and H100T OS Management Hub Agent:
...
- Enabled in Oracle UK Sovereign Cloud.
...
The[diagnostic tool](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins-troubleshooting.htm#diagnostic)is now included in the release package.
...
Cloud Guard Workload Protection:
- Added the Windows wlp-doctor script to the plugin for customer diagnostics.

Fleet Application Management Service:
- Added DualStack endpoint and IPv6 SingleStack support with IPv6-capable executor handling.
- Addressed CVE-2025-58189, CVE-2025-58187, CVE-2025-58188, CVE-2025-58185, and CVE-2025-61723, and fixed Windows startup by deferring Resource Principal authentication until the service client is needed.
...
Oracle Cloud Agent:
- Added support to configure the Windows plugin port range through`agent.yml`. Default behavior is unchanged unless these settings are explicitly configured.
1.59.0 April 03, 2026
...
Cloud Guard Workload Protection:
- Added the`wlp-doctor`script to the plugin for customer ease of access.
1.58.0 March 04, 2026

Fleet Application Management Service:
- Limited`stdout`content to the most recent 4kb to prevent buggy behavior on larger output.
1.57.0 February 05, 2026
- Monthly Patching.
1.56.0 January 12, 2026

Fleet Application Management Service:
- Enablement in OC2 and OC3 realms.
1.55.0 November 06, 2025
...
Custom Logs Monitoring:
- Improved logging for windows service errors.
1.54.0 August 06, 2025

Weblogic Management Service:
- Enable WLMS in all realms.

Custom Logs Monitoring:
- UM plugin onboarding for Windows 2025 and transition from a FIPS to non-FIPS package.

Fleet Application Management Service:
- Resource Principal AuthN and AuthZ implementation.

Oracle Cloud Agent:
- Use Microsoft FIPS-compliant golang compiler.
1.53.0 June 05, 2025
Cloud Guard Workload Protection:
- Optimizations to installation on Windows for better efficiency.
- Improves plugin stability when restarting on Windows.
Fleet Application Management Service:
- Bug fix for Windows task executions with custom runbooks.
1.52.0 May 07, 2025 OS Management Service Agent:
- Minimizes OS Management Service Agent downtime when you restart OCA
- Adds a periodic check that stops orphaned osmh-agent processes
- 

Removes OS Management Service Agent (OSMS) and Autonomous Linux (ALX) as they are no longer supported as of April 23, 2025
1.51.0 April 07, 2025 Fleet Application Management:
- Introduced support for Fleet Application Management (FAMS) plugin, now available in select regions
- Support will be expanded to more regions over time OS Management Service Agent
- Improves reliability of the`osmh-agent`auto-upgrade process
1.50.0 March 11, 2025 Cloud Guard Workload Protection:
- Cloud Guard Workload Protection Agent no longer uninstalls when it shuts down
1.49.0 February 13, 2025

Block Volume Management:
- Resolves a bug that overrode iscsid.conf noop parameters, this fix reduces the chance of iscsid to stop responding OS Management Service Agent
- Supports Boostrapper image downloads in ONSR
- Adds automatic upgrade for osmh-agent Cloud Guard Workload Protection:
- Adds a rollback when a new version of the wlpagent installation fails
1.48.0 January 8, 2025 OS Management Service Agent
- Fixes issue where the binary failed to recover after a new install Custom Logs Monitoring:
...
- Windows machines will no longer register failure events in cases of IMDS being unreachable Compute Instance Run Command:
- Run command reverts to main endpoint fix Custom Logs Monitoring:
...
Removes installer packages from diagnostic tool results.
...
Adds all Windows Server 2019 users to the[diagnostic tool](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins-troubleshooting.htm#diagnostic).

Fixes to the[diagnostic tool](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins-troubleshooting.htm#diagnostic).
1.15.0 September 21, 2021 Bug fixes.
1.14.0 August 20, 2021

The[diagnostic tool](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins-troubleshooting.htm#diagnostic)is now included in the release package.

Added support for the OS Management Service Agent self-test.
1.13.0 July 1, 2021
...
Closes open handle for allocstall metric.
...
Custom Logs Monitoring plugin enabled in US Government Cloud realms.
...
IPv6 Support: Oracle Cloud Agent and most of its plugins now support IPv6 single-stack environments. See the preceding release notes for details about support by specific plugins.

The following plugins do not currently support IPv6 single-stack:
- Bastion
- WebLogic Management Service
- Cloud Guard Workload Protection
...
For troubleshooting steps, see[Troubleshooting Oracle Cloud Agent](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/manage-plugins-troubleshooting.htm).
...
- [Connect to the instance](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/accessinginstance.htm).
- 
