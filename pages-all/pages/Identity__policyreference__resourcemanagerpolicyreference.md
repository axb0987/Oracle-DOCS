...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/policyreference/resourcemanagerpolicyreference.htm
...
String Use this variable to limit access to specified stacks. For example, use the following phrase:`where any {target.stack.id = ocid1.ormstack.uniqueid1, target.stack.id = ocid1.ormstack.uniqueid2}`
...
`ORM_CONFIG_SOURCE_PROVIDER_INSPECT``ListConfigurationSourceProviders`
...
`ORM_CONFIG_SOURCE_PROVIDER_READ``GetConfigurationSourceProvider``CreateStack`: When creating stacks that use configuration source providers (`configSourceType`value`GIT_CONFIG_SOURCE`), also need`manage orm-stacks`
use
...
`CancelJob``CreateJob`(also need`use orm-stacks`)
...
no extra`CreateJob`(also need`manage orm-jobs`)
...
`ListTerraformVersions``CreateStack`: When creating stacks that use configuration source providers (`configSourceType`value`GIT_CONFIG_SOURCE`), also need`read orm-config-source-providers`
...
`ORM_WORK_REQUEST_INSPECT``ListWorkRequests`
...
If using configuration source providers (`configSourceType`value`GIT_CONFIG_SOURCE`), also need`ORM_CONFIG_SOURCE_PROVIDER_READ`
