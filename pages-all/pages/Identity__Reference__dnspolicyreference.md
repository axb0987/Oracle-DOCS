...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/Reference/dnspolicyreference.htm
...
DNS_ZONE_READ`GetZone``GetZoneRecords`
use

READ +

DNS_ZONE_UPDATE`UpdateZone`
...
DNS_STEERING_POLICY_INSPECT`ListSteeringPolicies`
...
DNS_STEERING_POLICY_READ`GetSteeringPolicy`
...
DNS_POLICY_STEERING_UPDATE`UpdateSteeringPolicy`
...
DNS_STEERING_ATTACHMENT_INSPECT`ListSteeringPolicyAttachments`
...
DNS_STEERING_ATTACHMENT_READ`GetSteeringPolicyAttachment`
...
DNS_TSIG_KEY_INSPECT`ListTsigKeys`
...
DNS_TSIG_KEY_UPDATE`UpdateTsigKey`
...
DNS_RESOLVER_INSPECT`ListResolvers`
...
DNS_RESOLVER_UPDATE`UpdateResolver`
...
DNS_RESOLVER_ENDPOINT_INSPECT`ListResolverEndpoints`
...
DNS_RESOLVER_ENDPOINT_READ`GetResolverEndpoint`
...
DNS_RESOLVER_ENDPOINT_UPDATE`UpdateResolverEndpoint`
...
For a private zone, the DNS_VIEW_INSPECT permission is also required.

If the request contains`TsigKeys`in`externalMasters`or`externalDownstreams`, then the DNS_TSIG_KEY_READ permission is required for each TsigKey.
...
Also requires permission to retrieve the Health Check Monitor if one is specified. See[Details for Health Checks](https://docs.oracle.com/en-us/iaas/Content/Identity/Reference/healthcheckpolicyreference.htm#top)for more.
...
If the request contains`attachedViews`, then the DNS_VIEW_INSPECT permission is required for each View.
