...
- Source: https://docs.oracle.com/en-us/iaas/Content/Network/Reference/ciscoasaCPEpolicybased.htm
- Fetched: 2026-09-05 02:42 CDT

# Cisco ASA: Policy-Based

This topic provides a policy-based configuration for a Cisco ASA running software version 8.5 to 9.7.0.
Important  
...
With policy-based configuration, you can configure only a single tunnel between the Cisco ASA and the Dynamic Routing Gateway (DRG).
...
VPN filters let you further filter traffic either before it enters or after it exits a tunnel. Use VPN filters if you need more granularity for filtering different traffic types or source/destination flows. For more information, see Cisco's[VPN Filter documentation](https://www.cisco.com/c/en/us/support/docs/security/pix-500-series-security-appliances/99103-pix-asa-vpn-filter.html).

VPN filter configuration isn't included in the configuration template that appears in the[CPE Configuration](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/ciscoasaCPEpolicybased.htm#CPE)section. To use VPN filters, add the following configuration items manually.
- 

Access control list (ACL): Create an ACL that the VPN filter can use to restrict the traffic allowed through the tunnels. If you have an ACL already used for a VPN filter, don't also use it for an interface access group.
...
Group policy: Apply the VPN filter to the group policy.
...
Tunnel group: Apply the group policy to the tunnel group.
...
We recommend that you always keep interesting traffic running through the IPSec tunnels if the CPE supports doing so. Cisco ASA requires that you configure SLA monitoring, which keeps interesting traffic running through the IPSec tunnels. Often where Cisco ASA acts as the initiator phase 2 doesn't come up until there is no interesting traffic, which causes the SA to be down either when the tunnel is brought up or after a security association rekey. To avoid such situations, continuous interesting traffic or features like IP SLA can be used. For more information, see the section for "IP SLA Configuration" in the[Cisco ASA policy-based configuration template](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/ciscoasaCPEpolicybased.htm#CPE).
...
Use the following command to change the MSS. This command isn't part of the sample configuration in the[CPE Configuration](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/ciscoasaCPEpolicybased.htm#CPE)section of this topic. Apply the TCP MSS adjustment command manually, if needed.
...
Set the DF bit (recommended): Packets have the DF bit set in their IP header. The ASA might still fragment the packet if the original received packet cleared the DF bit.
...
If VPN traffic enters an interface with the same security level as an interface toward the packet's next hop, you must allow that traffic. By default, the packets between interfaces that have identical security levels on the ASA are dropped.

Add the following command manually if you need to allow traffic between interfaces with the same security levels. This command isn't part of the sample configuration in the[CPE Configuration](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/ciscoasaCPEpolicybased.htm#CPE)section.
...
Cisco ASA versions 9.7.1 and newer support[route-based configuration](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/ciscoasaCPEroutebased.htm), which is the recommended method to avoid interoperability issues.

If you want tunnel redundancy with a single Cisco ASA device, you must use the route-based configuration. With policy-based configuration, you can configure only a single tunnel between the Cisco ASA and the Dynamic Routing Gateway (DRG).

The configuration template refers to these items that you must provide:
- CPE public IP address: The internet-routable IP address assigned to the external interface on the CPE. You or the tenancy administrator provides this value to Oracle when creating the CPE object in the Oracle Console.
...
- Ensure that you allow traffic between the ASA and the Oracle VCN (the following configuration template references this access list with the variable`${outboundAclName}`).
...
- Identify the transform set used for the crypto map (the following configuration template references this transform set as`oracle-vcn-transform`).
...
This following configuration template from Oracle Cloud Infrastructure is a starting point for what you need to apply to the CPE. The syntax for each CPE device configuration can be different and depends on the model and software versions. Be sure to compare the CPE model and version to the appropriate configuration template.

Some parameters referenced in the template must be unique on the CPE, and the uniqueness can only be determined by accessing the CPE. Ensure that the parameters are valid on the CPE and don't overwrite any configured values. In particular, ensure that the following values are unique:
...
Oracle provides separate configuration templates for IKEv1 and IKEv2.

Oracle also provides a tool that can generate the template for you, with some information automatically filled in. For more information, see[Using the CPE Configuration Helper](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/../Tasks/CPEconfigurationhelper.htm).

[IKEv1 Configuration Template](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/ciscoasaCPEpolicybased.htm#)

[View the IKEv1 configuration template in full screen for easier reading.](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/../non-dita/vpn_cpe_ciscoasa_policy_based.txt)
...
[View the IKEv2 configuration template in full screen for easier reading.](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/../non-dita/vpn_cpe_ciscoasa_ikev2_policy_based.txt)
...
The following ASA commands are included for basic troubleshooting. For more exhaustive information, see Cisco's[IPSec Troubleshooting](https://www.cisco.com/c/en/us/support/docs/security-vpn/ipsec-negotiation-ike-protocols/5409-ipsec-debug-00.html)document.
...
Use the following command to verify the status of all BGP connections.
