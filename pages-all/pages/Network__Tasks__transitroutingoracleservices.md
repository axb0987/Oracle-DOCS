...
- Source: https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/transitroutingoracleservices.htm
...
- The VCN uses a[dynamic routing gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingDRGs.htm)(DRG) to communicate with the on-premises network. Access to Oracle services is through a[service gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/servicegateway.htm)on the VCN. The traffic from the VCN to the Oracle service travels over the Oracle network fabric and never traverses the public internet.
...
- Hosts in your VCN:
- [Private access through a service gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/servicegateway.htm): The VCN's hosts use private IP addresses.
...
If you use Oracle Analytics Cloud so that it initiates connection requests to clients, and you also want to set up private access to Oracle services for your on-premises network, see this[known issue](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Reference/known_issues_for_networking.htm#sgw-private-access).
...
DRG attachment:
- The VCN route table is associated with the DRG attachment . Why the attachment and not the DRG itself? Because the DRG is a standalone resource that you can attach to any VCN in the same region and tenancy as the DRG. The attachment itself identifies which VCN.
...
Callout 1: VCN route table for the DRG attachment
...
Callout 2: VCN route table for the service gateway
...
Callout 1: Route table associated with Subnet-Frontend
Destination CIDR Route Target
172.16.0.0/12 DRG

Callout 2: Route table associated with Subnet-Backend
Destination CIDR Route Target
All OSN services in the region Service gateway
...
Callout 4: Route table associated with service gateway
Destination CIDR Route Target
...
DRG attachment:
- This VCN route table is associated with the DRG attachment . Why the attachment and not the DRG itself? Because the DRG is a standalone resource that you can attach to any VCN in the same region and tenancy as the DRG. The attachment itself identifies which VCN.
...
- In the Console, view the VCN's details.
...
Callout 1: Route table associated with DRG attachment
Destination CIDR Route Target
All OSN services in the region Service Gateway

Callout 2: Route table associated with service gateway
Destination CIDR Route Target
172.16.0.0/12 DRG
...
Associate the route table (called VCN ingress route table in this example) with the VCN's DRG attachment:
- While still viewing the VCN's details, click Dynamic Routing Gateways to view the attached DRG.
...
Create a route table for the service gateway:
- While still viewing the VCN's details, click Route Tables .
- Click Create Route Table .
- 
...
Associate the route table (called Service Gateway Route Table in this example) with the service gateway:
- While still viewing the VCN's details, click Service Gateways .
- For the service gateway, click the Actions menu (three dots) , and then click Associate With Route Table .
- 
...
- If you haven't already, create the instance in the VCN. See[Creating an Instance](https://docs.oracle.com/iaas/Content/Compute/Tasks/launchinginstance.htm). The primary VNIC is created in the subnet you specify.
