# Synchronizing Users
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/applications/synchronize-user-accounts.htm
...
- This table summarizes the result of successfully running an import. For each SaaS app account, it shows whether there exists a matching identity domain user and the action that you need to perform to link the SaaS user account to an identity domain user.
...
Situation
Lists whether a matching identity domain user exists or doesn't exist based on your synchronization configuration:
- 
...
Shows the email address and username of the identity domain user.
Action
If there's no matching identity domain user, you need to select the appropriate action from the list:
- 
