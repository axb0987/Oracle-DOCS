...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/policyreference/filestoragepolicyreference.htm
...
EXPORT_SET_INSPECT`ListExportSets`
...
FILE_SYSTEM_INSPECT`ListFileSystems`
...
FILE_SYSTEM_MANAGE_SNAPSHOT_LOCK_GOVERNANCE
...
MOUNT_TARGET_INSPECT`ListMountTargets`
...
OUTBOUND_CONNECTOR​_INSPECT`ListOutboundConnectors`
...
REPLICATION_INSPECT`ListReplications`
...
[fss-work-requests](https://docs.oracle.com/en-us/iaas/Content/Identity/policyreference/filestoragepolicyreference.htm#)

Verbs Permissions APIs Fully Covered APIs Partially Covered
inspect

FSS_WORK_REQUEST_INSPECT`ListWorkRequests`
...
If a group uses the Console to create file systems, permissions to read mount targets is required.[See the File Storage policy examples](https://docs.oracle.com/en-us/iaas/Content/Identity/policyreference/../policiescommon/commonpolicies.htm#create-file-system)for further guidance.
...
If the request includes snapshot lock settings, you also need`FILE_SYSTEM_MANAGE_SNAPSHOT_LOCK_GOVERNANCE`(governance mode) or`FILE_SYSTEM_MANAGE_SNAPSHOT_LOCK_COMPLIANCE`(compliance mode).
`UpdateFilesystemSnapshotPolicy`

FILESYSTEM_SNAPSHOT_POLICY_UPDATE

If the request changes or removes snapshot lock settings, you also need`FILE_SYSTEM_MANAGE_SNAPSHOT_LOCK_GOVERNANCE`(governance mode) or`FILE_SYSTEM_MANAGE_SNAPSHOT_LOCK_COMPLIANCE`(compliance mode).
`AddFilesystemSnapshotPolicyLock`
...
If the request sets, changes, or removes snapshot lock settings, you also need`FILE_SYSTEM_MANAGE_SNAPSHOT_LOCK_GOVERNANCE`(governance mode) or`FILE_SYSTEM_MANAGE_SNAPSHOT_LOCK_COMPLIANCE`(compliance mode).
