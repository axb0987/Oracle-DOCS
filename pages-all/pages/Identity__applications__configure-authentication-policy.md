...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/applications/configure-authentication-policy.htm
...
- On the Integrated applications list page, select the enterprise application to change. If you need help finding the list page, see[Listing Applications](https://docs.oracle.com/en-us/iaas/Content/Identity/applications/listing-apps.htm).
- On the Application details page, select SSO configuration , and then select Edit SSO configuration . In the Authentication policy section, Check Audience validation , Allow CORS , or Require secure cookies , if needed.
...
- Check Add managed resources .
- In the Add managed resource window, select the resource for which you want to configure an authentication policy from the list of resources that you created in the Resources section or add a managed resource.
- Use the following table to define the Authentication method for the resource you have selected:
...
Same as Multitoken , but if the`Authorization`header isn't`Basic`,`Bearer`, or`session`, then instead of presenting the`401 Unauthoized`HTTP error, request App Gateway acts as authentication method was Basic Auth .
...
For example, you can use this method to disable access to a protected URL that's available in the application but you don't want users to access it.
