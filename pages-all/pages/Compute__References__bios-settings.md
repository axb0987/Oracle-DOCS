...
- Source: https://docs.oracle.com/en-us/iaas/Content/Compute/References/bios-settings.htm
...
Disabling cores by configuring advanced BIOS settings isn't a valid means for determining or limiting the number of Oracle software licenses required for a bare metal instance in a bring your own license scenario.
...
You can disable cores to use fewer cores than the full size of the[shape](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm). The instance itself is billed for the full shape, regardless of whether all cores are enabled.
...
On Arm CPUs, you can configure the number of NUMA nodes per physical processor chip. The following options are available:
- NPS1: Monolithic. Each processor chip is a NUMA node. This is the default.
- NPS2: Hemisphere. Each processor chip is divided into two NUMA nodes.
- NPS4: Quadrant. Each processor chip is divided into four NUMA nodes.
...
- Click Change shape .
- Select a bare metal shape, and then click Show advanced BIOS settings . Select the options that you want to configure. The settings that are available depend on the shape.
- Click Select shape .
- Finish creating the instance, and then click Create .

### Using the API

Use the[LaunchInstance](https://docs.oracle.com/iaas/api/#/en/iaas/latest/Instance/LaunchInstance)operation, specifying the BIOS settings in the`platformConfig`
