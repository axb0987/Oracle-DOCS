...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/policyreference/corepolicyreference_topic-Permissions_Required_for_Each_API_Operation.htm
...
`CreateComputeGpuMemoryCluster`COMPUTE_GPU_MEMORY_CLUSTER_CREATE and COMPUTE_CLUSTER_READ and COMPUTE_CLUSTER_UPDATE and INSTANCE_CREATE and COMPUTE_GPU_MEMORY_FABRIC_READ
`ChangeComputeGpuMemoryClusterCompartment`COMPUTE_GPU_MEMORY_CLUSTER_MOVE
`DeleteComputeGpuMemoryCluster`COMPUTE_GPU_MEMORY_CLUSTER_DELETE and INSTANCE_DELETE
`GetComputeGpuMemoryCluster`COMPUTE_GPU_MEMORY_CLUSTER_READ
`ListComputeGpuMemoryClusters`COMPUTE_GPU_MEMORY_CLUSTER_INSPECT and COMPUTE_CLUSTER_INSPECT
`UpdateComputeGpuMemoryCluster`

COMPUTE_GPU_MEMORY_CLUSTER_UPDATE and COMPUTE_CLUSTER_UPDATE and INSTANCE_CREATE
`ListComputeGpuMemoryClusterInstances`COMPUTE_GPU_MEMORY_CLUSTER_INSPECT and COMPUTE_CLUSTER_INSPECT
`ChangeComputeGpuMemoryFabricCompartment`COMPUTE_GPU_MEMORY_FABRIC_MOVE
`GetComputeGpuMemoryFabric`COMPUTE_GPU_MEMORY_FABRIC_READ
`ListComputeGpuMemoryFabrics`COMPUTE_GPU_MEMORY_FABRIC_INSPECT and COMPUTE_HPC_ISLAND_INSPECT and COMPUTE_NETWORK_BLOCK_INSPECT
`UpdateComputeGpuMemoryFabric`COMPUTE_GPU_MEMORY_FABRIC_UPDATE
`ListConsoleHistories`CONSOLE_HISTORY_READ and INSTANCE_INSPECT
...
`ListFirmwareBundles`FIRMWARE_BUNDLE_INSPECT
`GetFirmwareBundle`FIRMWARE_BUNDLE_READ
...
`BulkCreateIpv6s`IPV6_CREATE and SUBNET_ATTACH and VNIC_ASSIGN
`DeleteIpv6`IPV6_DELETE and SUBNET_DETACH and VNIC_UNASSIGN
`BulkDeleteIpv6s`IPV6_DELETE and SUBNET_DETACH and VNIC_UNASSIGN
...
`BulkUpdatePrivateIps`PRIVATE_IP_UPDATE and VNIC_ASSIGN and VNIC_UNASSIGN
`CreatePrivateIp`
...
`BulkDeletePrivateIps`PRIVATE_IP_DELETE and PRIVATE_IP_UNASSIGN and VNIC_UNASSIGN and SUBNET_DETACH
