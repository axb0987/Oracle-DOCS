# Configure Integration Between Oracle Access Governance and Workday
- Source: https://docs.oracle.com/en-us/iaas/Content/access-governance/configure-integration-between-oracle-access-governance-and-workday.htm
...
Before you install and configure a Workday Orchestrated System, you should consider the following prerequisites and tasks.
- Check that your Workday system is certified with Oracle Access Governance by referring to[Components Certified for Integration with Oracle Access Governance](https://docs.oracle.com/en-us/iaas/Content/access-governance/workday-integration-reference.htm#workday-components-certified-for-integration-with-oracle-access-governance)for details of the versions supported.

## Configure

You can establish a connection between Workday and Oracle Access Governance by entering connection details. To achieve this, use the orchestrated systems functionality available in the Oracle Access Governance Console.
...
Enter details of the connection to your Workday system.
- Select which Workday workers you want to retrieve when performing a data load. Choose from one of the following values:
- Only workers with an account on Workday
- All workers
Note  
  
A worker is a generic term for any user registered in the Workday system. A worker with account only is a specific type of worker who has a Workday account for certain purposes. This might include contractors, consultants, or other non-employee individuals who need access to Workday for specific tasks, like accessing benefits information or timekeeping, without being directly employed,
- 

On the Integration settings step of the workflow, enter the details required to allow Oracle Access Governance to connect to your Workday system.

Integration settings
Parameter Name Mandatory? Description
What is the tenant name? Yes The Workday tenant name. This is a unique identifier for a specific Workday application instance. The tenant name can be found in the Workday Human Resources WSDL URL, for example`https://<domain_name>/ccx/service/ <tenant> /Human_Resources/v1`
What is the domain or server instance where Workday sourcing services are hosted? Enter the domain name or server instance name (hostname) for your Workday instance. Refer to the Workday documentation on how to get these values.
What is the username? Enter the username of the Workday user you want to use when integrating with Oracle Access Governance.
What is the password? Enter the password for the Workday user you are using to integrate with Oracle Access Governance
