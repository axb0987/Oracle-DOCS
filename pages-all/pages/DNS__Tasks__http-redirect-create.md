...
- Source: https://docs.oracle.com/en-us/iaas/Content/DNS/Tasks/http-redirect-create.htm
...
- On the HTTP redirects list page, select Create HTTP Redirect . If you need help finding the list page, see[Listing HTTP Redirects](https://docs.oracle.com/en-us/iaas/Content/DNS/Tasks/http-redirect-list.htm).
- (Optional) Enter a descriptive name for the redirect. Avoid entering confidential information.
- (Optional) Select a zone from a list of zones that are already configured.
If the Create DNS Record checkbox is selected, the zone is used to build records for the redirect.
- Enter the domain name from which traffic is redirected.
- In the Target: section, enter the information for the redirect endpoint.
...
If you don't check the Create DNS record now, it's not possible to edit the redirect to obtain the record for`redirect.waf.oci.oraclecloud.net`. You must either delete the redirect and start over, or manually add the record to the DNS.
...
- To create the HTTP redirect now, select Create .
- To create the redirect later using Resource Manager and Terraform, select Save as Stack to save the resource definition as a Terraform configuration.
...
Run the[CreateHttpRedirect](https://docs.oracle.com/iaas/api/#/en/waas/latest/HttpRedirect/CreateHttpRedirect)
