...
- Source: https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm
...
- 
From the main Google Cloud portal:
- Expand the main menu from the upper left corner
- Scroll down to Hybrid Connectivity
- Select VPN
- 

On the next page, select the Create VPN Connection link to start the workflow to create a IPSec VPN connection.
- 

For VPN Options , select High-availability (HA) VPN then select the Continue button at the bottom.
- 

Create a Cloud HA VPN gateway. Enter the following details:

- Name: Give the VPN gateway a name.
- Network: Select the VPC that the IPSec VPN connects to.
- 

Region: Select the region for the VPN gateway

When you're finished configuring the VPN gateway, select the Create &amp; Continue button to continue.
- 

The next page exposes the public IP of the GCP VPN endpoint.
...
- Static Route CIDR: Enter a default route, 0.0.0.0/0. Because BGP is used for the active tunnel, OCI ignores this route. This entry is required for the second tunnel of the IPSec connection which by default uses static routing and is unused in this scenario. If you plan to use static routing for this connection, input static routes representing the GCP virtual networks. Up to 10 static routes can be configured for each IPSec connection.
- 
...
- Provide custom shared secret: The pre-shared key used by IPSec for this tunnel. Select this checkbox if you would like to use a custom key. If no selection is provided, one is generated for you.
...
- Select Show advanced options for the tunnel and change the Phase 1 and Phase 2 lifetimes to match the GCP side as listed here[https://cloud.google.com/network-connectivity/docs/vpn/concepts/supported-ike-ciphers](https://cloud.google.com/network-connectivity/docs/vpn/concepts/supported-ike-ciphers).

Phase 1 and Phase 2 lifetimes have different values on the GCP side depending on if you're using IKEv1 or IKEv2. These are:
...
After the IPSec connection has been provisioned, save the Site-to-Site VPN IP address to use as the CPE IP in the GCP portal and the shared secret for the tunnel.
- 

Browse to the IPSec connection in the OCI Console.

Decide which tunnel to use as the primary. Save the Site-to-Site VPN IP address of that tunnel. Next, select the tunnel name of that tunnel to be taken to the tunnel view.
- Under the tunnel, select the link to view the shared secret. Save it. The shared secret is used to complete the IPSec VPN configuration in GCP.

[GCP - Create a VPN Peer Gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#)

- Return to the GCP VPN configuration window.
- Under Peer VPN Gateway , select On-prem or Non Google Cloud .
- Expand the Peer VPN gateway name list and select Create new VPN peer gateway .
- You're taken to the Add a peer VPN gateway page. Enter the following details:

- Name: Give the peer VPN gateway a name.
...
- Select the Create button at the bottom to continue. You're taken back to the Create a VPN workflow.

[GCP - Create a Cloud Router](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#)

- 

Expand the Cloud Router list and select Create new router .
- 

You're taken to the Create a router page. Enter the following details:
- Name: Give the cloud router a name.
- 

Google ASN: Input the Google BGP ASN, also used when configuring the IPSec connection in OCI.

Leave all other options as default.
- When you're finished configuring the cloud router, select the Create button to continue.

You're taken back to the Create a VPN workflow.

[GCP - Complete configuring VPN Tunnel](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#)

Complete configuring the VPN tunnel. Enter the following details:
...
- Associated peer VPN Gateway interface: Match the Oracle VPN IP address of the OCI VPN tunnel. See[OCI - Save Oracle VPN IP Address and Shared Secret](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#vpn_to_gcp_oci_save_oracle_vpn_ip_address_and_shared_secret).
...
IKE pre-shared key: Match the shared secret of the VPN tunnel in the IPSec connection in OCI. See[OCI - Save Oracle VPN IP Address and Shared Secret](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#vpn_to_gcp_oci_save_oracle_vpn_ip_address_and_shared_secret). Leave all other options as default.
...
This step involves configuring the BGP session for an IPSec tunnel. The BGP settings need to match the OCI side of the configuration configured in[OCI - Create IPSec Connection](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#vpn_to_gcp_oci_create_ipsec_connection).
- Select the Configure button under BGP Session for the VPN tunnel.
- In the Create BGP Session page for the tunnel, enter the following details:
- Name: Give the BGP session a name.
...
- Cloud Router BGP IP: Match the IP address configured for IPV4 Inside Tunnel Interface - CPE of tunnel 1 in[OCI - Create IPSec Connection](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#vpn_to_gcp_oci_create_ipsec_connection). Don't use CIDR notation in this field.
- BGP peer IP: Match the IP address configured for IPV4 Inside Tunnel Interface - Oracle of tunnel 1 in[OCI - Create IPSec Connection](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#vpn_to_gcp_oci_create_ipsec_connection). Don't use CIDR notation in this field.
- 

BGP peer: Select Enabled .

Leave all other options as default.
- When you finish configuring the BGP session, select Save &amp; Continue to go back to the Configure BGP sessions page.
- 

Select the Save BGP configuration button to continue.
- You're taken to the Summary and reminder page.

The VPN tunnel status and BGP status displays as "established."
- Select OK at the bottom to complete the configuration.

[Verification](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/vpn_to_gcp.htm#)

Browse to the IPSec connection in OCI and the Site-to-Site VPN connections in GCP to verify status of the tunnel.
