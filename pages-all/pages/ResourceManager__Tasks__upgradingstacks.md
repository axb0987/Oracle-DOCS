...
- Source: https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/upgradingstacks.htm
...
For information about Terraform versions supported by Resource Manager, see[Supported Terraform Versions](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/../Reference/terraformversions.htm).
...
To support deprecation of[older Terraform versions (earlier than 1.5.x)](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/../Reference/terraformversions.htm), Resource Manager will try an automatic Terraform version upgrade. This automatic upgrade will be attempted whenever you run[a successful apply job](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/create-job-apply.htm)(job state is succeeded) on a stack that's configured for an older Terraform version.
Note  
  
The target version for the attempted upgrade depends on the stack's currently configured Terraform version. Intermediary automatic upgrades move the stack toward 1.5.x in one or more successful upgrade attempts. Using intermediary upgrades works around differences in HCL syntax, provider block definitions, and state file differences, as discussed in the manual upgrade instructions. The target version for an upgrade is selected as follows:
- The target version to upgrade 0.12.x is 0.13.x.
- The target version to upgrade 0.13.x is 0.14.x.
- The target version to upgrade 0.14.x, 1.0.x, 1.1.x, or 1.2.x is 1.5.x.

The upgrade attempt only occurs after an apply job succeeds (job state is succeeded).
- If the upgrade succeeds, then the stack's Terraform version and state file are upgraded to 1.5.x (or intermediary version). No further action is required.
- If the upgrade fails, then the stack's Terraform version is unchanged, and resources aren't affected. An upgrade failure message appears in the stack's details page. In this situation,[get logs for the apply job](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/get-job-logs.htm)(select Show terraform upgrade logs for information about the failure) and then update the Terraform configuration. The next successful apply job that you run on this stack will trigger the automatic upgrade process by Resource Manager.
Note  
  
Even if the upgrade fails, the job state is still succeeded.
...
See Terraform's official upgrade guides:
- [Upgrading to Terraform v0.13](https://developer.hashicorp.com/terraform/language/v1.1.x/upgrade-guides/0-13)
- [Upgrading to Terraform v0.14](https://developer.hashicorp.com/terraform/language/v1.1.x/upgrade-guides/0-14)
...
For CLI and API instructions to get a stack's details, see[Getting a Stack's Details](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/get-stack.htm).
...
For CLI and API instructions, see[Creating a Plan Job](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/create-job-plan.htm).
- 
...
For CLI and API instructions, see[Getting Logs for a Job](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/get-job-logs.htm).
...
For CLI and API instructions, see[Creating an Apply Job](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/create-job-apply.htm).

When the action finishes, the details page for the related job opens.
- 

Confirm that the apply action was successful: Select Logs to review the log for the completed plan action.
...
- In the Console: On the Stacks list page, select the stack that you want to work with. If you need help finding the list page or the stack, see[Listing Stacks](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/../Tasks/list-stacks.htm).
- Next to Terraform configuration , select Download .

For CLI and API instructions, see[Getting a Stack's Terraform Configuration](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/get-stack-tf-config.htm).
...
For CLI and API instructions, see[Getting a Stack's State File](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/get-stack-tf-state.htm).
...
- On the same computer that you used to[store the downloaded Terraform configuration and state files](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/upgradingstacks.htm#download), download the`.zip`files needed to upgrade your Terraform configuration:

- 

From[https://releases.hashicorp.com/terraform](https://releases.hashicorp.com/terraform), download the`.zip`file for your target Terraform version and operating system.

For example, for Terraform 0.13 on Mac, download the Darwin version:[https://releases.hashicorp.com/terraform/0.13.7/terraform_0.13.7_darwin_amd64.zip](https://releases.hashicorp.com/terraform/0.13.7/terraform_0.13.7_darwin_amd64.zip).
...
- To follow code examples in the rest of this procedure, rename the extracted file as`terraform_ <major-version>`.
Example:`terraform_13`
- To make the command (extracted file) accessible, store it in a directory that's present in your PATH directory.
- Open a command prompt.
- Change directory to the folder where you stored the downloaded stack information.
Example:
...
- To upgrade the syntax of your Terraform configuration, run the command for the target Terraform version:
...
- Create a`.zip`archive of your Terraform configuration files.

Example`.zip`archive:`c:\StackUpgrade\MyConfigUpgraded.zip`

Ensure that the archive omits the Terraform state file (`terraform.tfstate`) and the`.terraform`directory to satisfy the[required file structure for Terraform configurations](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/../Concepts/terraformconfigresourcemanager.htm).
...
This task uses the Console. For CLI and API instructions for updating a stack, see[Updating a Stack](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/update-stack.htm).

- In the Console, reopen the details page for the stack that you're upgrading: On the Stacks list page, select the stack that you want to work with. If you need help finding the list page or the stack, see[Listing Stacks](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/../Tasks/list-stacks.htm).
- From the Actions menu (three dots) for the stack, select Edit .
- Upload the[upgraded Terraform configuration](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/upgradingstacks.htm#config)to the stack.
...
This task uses the Console. For CLI and API instructions, see[Creating an Import Job](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/create-job-import.htm).
...
In the log contents, check for issues described at[Troubleshooting Logs During an Upgrade](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/upgradingstacks.htm#troubleshooting).
- Resolve any listed issues by manually updating the Terraform configuration, as described at[Troubleshooting Logs During an Upgrade](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Tasks/upgradingstacks.htm#troubleshooting).
