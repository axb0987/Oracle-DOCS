...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/msadbridge/create-microsoft-active-directory-ad-bridge.htm
...
- On the Directory integrations list page, select Add . If you need help finding the directory integrations page, see[Listing Active Directory Bridges](https://docs.oracle.com/en-us/iaas/Content/Identity/msadbridge/list-ad-bridges.htm).
- In the Install a bridge for Microsoft Active Directory page, make a note of the identity domain URL, client ID, and client secret.
...
The client secret is encrypted for security reasons. To see the secret in clear text, select Show Secret . To regenerate a secret for the bridge, select Regenerate .
- Select Download .
IAM downloads the client for the bridge.
Note  
  
Don't close the Install a bridge for Microsoft Active Directory page. You'll need to reference the identity domain URL, client ID, and client secret when creating the bridge.
- To install the client for the AD bridge, double-click the`ad-id-bridge....exe`file.
The Welcome to AD Bridge Installer window appears.
- In the Language Selection area, select the language that you want to use to install the client for the AD Bridge, and then select OK .
...
You can only installe the AD bridge on a machine that's connected to a Microsoft Active Directory domain, otherwise installation will not proceed.
- If the Open File — Security Warning dialog box appears, then select Run . Otherwise, go to step 8.
...
In the Browse For Folder dialog box, select the directory where IAM installs the client.
- 
...
- If your organization has a firewall in place and requires communication to be handled using an HTTP Proxy Server, then select Use Proxy Server . If you select this checkbox, then provide the full path (or address) of the proxy server and the administrator credentials for connecting to the proxy server.
...
- Select Next .
- In the Specify Credentials dialog box:
- Provide the identity domain URL, client ID, and client secret.
...
- Select Next .
- In the Specify Microsoft Active Directory Credentials dialog box, provide the following connection details to the Microsoft Active Directory server:
- Username: The Microsoft Active Directory account that the bridge uses to access the Microsoft Active Directory server.
- Password: The password for the Microsoft Active Directory account.
- Use SSL: If you're connecting to the server through an SSL connection, then leave this checkbox selected. Otherwise, clear it.

Note  
  
We recommend that you keep the Use SSL checkbox selected because this results in a faster and more-secure connection. After you select or clear this checkbox, and install the client for the bridge, you can't modify this setting.
- Select Test .
