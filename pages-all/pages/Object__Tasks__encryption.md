...
- Source: https://docs.oracle.com/en-us/iaas/Content/Object/Tasks/encryption.htm
...
Each object is encrypted with its own data encryption key. Data encryption keys are always encrypted with a master encryption key that's assigned to the bucket. Encryption is enabled by default and can't be turned off. By default, Oracle manages the master encryption key.

In addition to this default encryption, you can use these strategies to encrypt data:
- Use client-side encryption to encrypt objects with their encryption keys before storing them in Object Storage buckets. An available option is to use the Amazon S3 Compatibility API, along with client-side object encryption support available in AWS SDK for Java.
...
- Assign an Oracle Cloud Infrastructure Vault master encryption key that you control and rotate on a schedule. For more information, see[Using Your Own Keys in Vault for Server-Side Encryption](https://docs.oracle.com/en-us/iaas/Content/Object/Tasks/encryption.htm#UsingYourKMSKeys).
...
Object Storage doesn't store your encryption keys. You are responsible for tracking the key that is associated with each object and rotating the key as necessary. If you lose your encryption key, you can't retrieve your object.

### Scope and Constraints

Understand the following scope and constraints regarding SSE-C:
- An SSE-C key can't be associated with a bucket and can only be used to encrypt individual objects.
...
- To delete or rename an SSE-C encrypted object, you don't need to specify your encryption key.
...
`opc-sse-customer-algorithm`Specifies "AES256" as the encryption algorithm.[CopyObject](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/Object/CopyObject)

[GetObject](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/Object/GetObject)

[HeadObject](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/Object/HeadObject)

[PutObject](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/Object/PutObject)

[CreateMultipartUpload](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/MultipartUpload/CreateMultipartUpload)

[UploadPart](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/MultipartUpload/UploadPart)
...
The examples in this section use the full syntax for parameters, for example`--namespace`and`--bucket-name`. Sometimes, there are shortened parameter terms that you can use instead of the full ones, for example`-ns`for`--namespace`and`-bn`for`--bucket-name`. The CLI online`--help`for a particular command displays the shortened parameters that you can use.

For information about using the CLI, see[Command Line Interface (CLI)](https://docs.oracle.com/iaas/Content/API/Concepts/cliconcepts.htm). For a complete list of flags and options available for CLI commands, see the[Command Line Reference](https://docs.oracle.com/iaas/tools/oci-cli/latest/oci_cli_docs/).

#### Using SSE-KMS Bucket Keys

Object Storage workloads that use customer-managed encryption keys with OCI Vault can be affected by Vault KMS request-rate limits at very high throughput. Each encrypted object read or write requires a KMS operation, sustained high request rates can add latency and could result in throttling. This is most relevant for large-scale analytics and emerging high-IOPS AI/ML workloads.

Bucket Key reduces per-object KMS calls for high-throughput Object Storage buckets that use SSE-KMS. A bucket key is a bucket-scoped intermediate encryption key protected by the customer's KMS key. Object Storage uses it to wrap data encryption keys for eligible objects, reducing KMS dependency while preserving SSE-KMS semantics. This reduces calls from Object Storage to KMS, lowers PUT and GET latency, and decreases the likelihood of KMS throttling. Customers retain the same compliance protections without changing applications that interact with Object Storage.
Note  
  

SSE-KMS Bucket Key is opt-in per bucket and off by default. Existing objects don't automatically move to bucket key wrapping when the bucket setting is enabled, you must run bucket re-encryption to migrate existing objects.

##### Prerequisites for using SSE-KMS Bucket Keys

- 

[The KMS key must be created](https://docs.oracle.com/iaas/Content/KeyManagement/Tasks/managingkeys_topic-To_create_a_new_key.htm).
- 

[The Object Storage service principal must have the required permissions to use the KMS key](https://docs.oracle.com/iaas/Content/bigdata/policies-kms-key-obj-storage.htm).
Note  
  

KMS keys are owned and managed by the customer and stored in the customer tenancy.
...
[PutObject](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/Object/PutObject)[CopyObject](https://docs.oracle.com/iaas/api/#/en/objectstorage/latest/Object/CopyObject)
...
See[Re-encrypting an Object](https://docs.oracle.com/en-us/iaas/Content/Object/Tasks/managingobjects_topic-To_reencrypt_an_object.htm)
