...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/applications/add-app-catalog-application.htm
...
- In the Add application window, select Application Catalog .
- Depending on the options you see, select either Launch workflow or Launch app catalog .
- Find the application that you want by choosing a category (predefined by Oracle), searching for the application name, or selecting an integration filter. By default, all applications are shown.
- Select the application that you want to add.
- Enter a name and optional description for the application.
...
- (Optional) If applicable, enter a custom sign-in URL, which is URL to which users are redirected to sign in. If you're using a default sign-in page provided by Oracle, leave this field blank.
- (Optional) If applicable, enter a custom sign-out URL, which is the URL to which users are directed after the sign-out process. If you're using a default sign-in page provided by Oracle, leave this field blank.
- (Optional) In the Custom error URL field, enter the error page URL to which a user is redirected, after a failure. If you don't provide a value, the tenant-specific error page URL is used. If neither of those error URLs is configured, then the user is redirected to the IAM error page (/ui/v1/error).
...
- In the Custom social linking callback URL field, enter the URL that IAM can redirect to after the linking of a user between social providers and IAM is complete.
...
- If you want the app to be listed in the Catalog , select User can request access .
...
- In the Authentication and authorization section, select Enforce grants as authorization if you want to allow access to this app only if the user has been granted this app.
- Select Show advanced options to add tags to the application.
If you have permissions to create a resource, then you also have permissions to apply free-form tags to that resource. To apply a defined tag, you must have permissions to use the tag namespace. For more information about tagging, see[Resource Tags](https://docs.oracle.com/iaas/Content/General/Concepts/resourcetags.htm). If you're not sure whether to apply tags, skip this option or ask an administrator. You can apply tags later.
- Perform one of the following actions, depending on the application.

- Select Create application . The application is added in a deactivated state. Skip to step 21.
- Select Next and configure SSO.
- To import the IAM signing signature into the application, select Download signing certificate . This certificate is used by the SAML application to verify that the SAML assertion is valid.

To get the issuing IAM root certificate, see[Getting the Root CA Certificate](https://docs.oracle.com/en-us/iaas/Content/Identity/applications/../defaultsettings/obtain-root-ca-certificate.htm).
- To import the IAM identity provider metadata into the application, select Download identity provider metadata . The SAML application needs this information so that it can trust and process the SAML assertion that's generated by IAM as part of the federation process. This information includes, for example, profile and binding support, connection endpoints, and certificate information.
- Complete the General , Additional configurations , and Attribute configuration sections, as necessary.
- Under Resources , select Users to assign users to the application, and select Groups to assign groups to the application.
