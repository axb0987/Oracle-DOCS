...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/conteng_ipv4-and-ipv6.htm
- Fetched: 2026-09-05 01:54 CDT

# Enabling Clusters for IPv4 and IPv6

Find out what you need to know about IPv4 and IPv6 addresses when using Kubernetes Engine (OKE).
...
The IPv4 internet protocol is a standard for identifying devices on a network by assigning a unique IP address to each device. IPv4 has an address space of 32 bits (2 32 ), which provides more than 4.2 billion unique IP addresses. An IPv4 address is a 32 bit value, commonly formatted as four segments of 8 bits (known as octets) separated by a dot, where each segment is a decimal value between 0 and 255. For example, 203.0.113.2.
...
IPv6 is the successor to IPv4 and has a much larger address space of 128 bits (2 128 ), which provides 3.4 x 10 38 unique IP addresses. An IPv6 address is a 128 bit value, formatted as eight groups of 16 bits (known as hectets), separated by colons, where each group is a hexadecimal value between 0000 and ffff. For example, 2001:0db8:0123:1111:abcd:ef01:2345:6789.
...
- the Console: Use the[Custom Create workflow](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengcreatingclusterusingoke_topic-Using_the_Console_to_create_a_Custom_Cluster_with_Explicitly_Defined_Settings.htm)to create the cluster. The cluster's IP address families are inferred from the selections you make.
...
- [Specifying IP Address Families for Load Balancers and Network Load Balancers](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengconfiguringloadbalancersnetworkloadbalancers-subtopic.htm#contengcreatingloadbalancer_topic_Specifying_IPAddressfamily)
- [Assigning Specific IPv4 and IPv6 Addresses to Network Load Balancers](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengconfiguringloadbalancersnetworkloadbalancers-subtopic.htm#contengcreatingloadbalancer_topic_NLB_IPv4_IPv6_address_assignment)
...
Additional Considerations for Pod Network Configuration

When configuring pod communication in an IPv4 or IPv6 environment, certain host settings can impact traffic forwarding. Specifically, the following kernel parameters might be disabled (set to`0`), affecting pod network traffic for their respective IP address families:
- `/proc/sys/net/ipv4/ip_forward`(for IPv4 pod traffic)
- `/proc/sys/net/ipv6/conf/all/forwarding`(for IPv6 pod traffic)

If these settings are disabled, pod communication for the corresponding IP address family cannot traverse the host's network interfaces. Attempts are made to enable these parameters by setting their values to`1`, ensuring proper pod traffic forwarding for both IPv4 and IPv6. However, custom host configurations (for example, files in`/etc/sysctl.d/`) might override these settings, leading to network interruptions.

If you observe issues with pod communication, such as traffic not being forwarded between pods or from pods to the host, verify the values of the relevant kernel parameters. Ensure they are set to`1`for the affected IP address family. Adjust any conflicting configurations in`/etc/sysctl.d/`or related files to resolve the issue.
