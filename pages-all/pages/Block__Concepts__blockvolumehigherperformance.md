# Higher Performance
- Source: https://docs.oracle.com/en-us/iaas/Content/Block/Concepts/blockvolumehigherperformance.htm
...
[Bare metal instances](https://docs.oracle.com/en-us/iaas/Content/Block/Concepts/overview.htm#BlockVolumeEncryption)that use in-transit encryption will see a maximum throughput of 540 MB/s at the Higher Performance level.
...
The information in this section only applies to volumes attached to Linux instances. You do not need to make any adjustments to the iSCSI queue depth configuration to achieve higher performance for volumes attached to Windows instances.
...
### To adjust the queue depth for a new block volume attachment to an instance
...
### To adjust the queue depth for an existing volume attachment to an instance

- 

Run the following command to update the queue depth for the volume's iSCSI to 128:
```
...
Run the following command to log out the iSCSI node:
```
...
Run the following command to log the iSCSI node:
```
