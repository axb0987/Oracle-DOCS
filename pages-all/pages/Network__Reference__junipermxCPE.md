# Juniper MX
- Source: https://docs.oracle.com/en-us/iaas/Content/Network/Reference/junipermxCPE.htm
- Fetched: 2026-09-05 02:42 CDT

# Juniper MX

This topic provides configuration for a Juniper MX running software version JunOS 15.0 (or newer).
Important  
...
The configuration template provided is for a Juniper MX router running JunOS 15.0 (or newer). The template provides information for each tunnel that you must configure. We recommend setting up all configured tunnels for maximum redundancy.
...
- You can distribute load across the ASICs by spreading tunnels across them.
...
- The router forwards packets from the on-premises network to the VCN into the inside unit.
...
You specify the IKE version when defining the IKE policy. In the following configuration, a comment shows how to configure the IKE policy to use IKEv1 or IKEv2.

### Configuration Template

[View the configuration template in full screen for easier reading.](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/../non-dita/vpn_cpe_juniper_mx.txt)
...
Use the following commands to check the routes advertised to and received from Oracle Cloud Infrastructure. If you already configured the CPE to use routing instances, use the commands with`table`&lt;table-name&gt; at the end.
