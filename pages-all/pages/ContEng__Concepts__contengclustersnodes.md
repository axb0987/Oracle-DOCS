...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengclustersnodes.htm
- Fetched: 2026-09-05 01:53 CDT

# Kubernetes Engine (OKE) and Kubernetes Concepts

Find out about the key concepts you need to understand before using Kubernetes Engine (OKE).

This topic describes key concepts you need to understand when using OKE.

## Kubernetes Engine (OKE)

OKE is a fully-managed, scalable, and highly available service for deploying containerized applications in the cloud using[Kubernetes](https://www.oracle.com/cloud-native/container-engine-kubernetes/what-is-kubernetes). Use OKE when your development team wants to reliably build, deploy, and manage cloud-native applications. You can choose to run applications on virtual nodes, managed nodes, or self-managed nodes, and OKE provisions them in an existing OCI tenancy.

OKE uses versions of Kubernetes certified as conformant by the[Cloud Native Computing Foundation (CNCF)](https://github.com/cncf/k8s-conformance). OKE is itself ISO-compliant (ISO-IEC 27001, 27017, 27018).

## Kubernetes

[Kubernetes](https://www.oracle.com/cloud-native/container-engine-kubernetes/what-is-kubernetes)is the open-source system for automating deployment, scaling, and management of containerized applications across clusters of hosts. The hosts (or nodes) in a cluster work together to run containerized workloads.

Kubernetes manages and schedules containers on these nodes. It organizes containers into logical units called pods, which are deployed and scaled based on application requirements. Pods simplify management, networking, and service discovery, making it easier to build and operate complex, distributed applications with Kubernetes.
...
When creating a cluster using OKE, you can create the new cluster as a basic cluster or as an enhanced cluster.

## Enhanced Clusters and Basic Clusters

When creating a new cluster with OKE, you specify the type of cluster to create. You can specify:
- Enhanced cluster: Enhanced clusters support all available features, including features such as virtual nodes, cluster add-on management, workload identity, and additional worker nodes per cluster. Enhanced clusters come with a financially-backed service level agreement (SLA).
- Basic cluster: Basic clusters support all the core functionality provided by Kubernetes and OKE. Basic clusters come with a service level objective (SLO), but not a financially-backed service level agreement (SLA).
...
For a more detailed comparison, see[Comparing Enhanced Clusters with Basic Clusters](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengcomparingenhancedwithbasicclusters_topic.htm).
...
Note that all references to 'clusters' in the OKE documentation refer to both enhanced clusters and basic clusters, unless explicitly stated otherwise.

## Kubernetes Cluster Control Plane and Kubernetes API

The Kubernetes cluster control plane implements core Kubernetes functionality. It runs on compute instances (known as 'control plane nodes') in the OKE service tenancy. The cluster control plane is fully managed by Oracle.
...
Access to the Kubernetes API is governed by multiple layers of security. You use OCI IAM policies to control who can use OKE to manage clusters, and obtain access to the cluster control plane itself. Within the cluster, you use Kubernetes RBAC to control what authenticated users and service accounts can do with the Kubernetes API (for example, when using kubectl).
Note  
...
When creating a node pool with OKE, you specify that the worker nodes in the node pool are to be created as one or other of the following:
...
See[Comparing Virtual Nodes with Managed Nodes](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengcomparingvirtualwithmanagednodes_topic.htm).

Note that all references to 'nodes' and 'worker nodes' in the OKE documentation refer to both virtual nodes and managed nodes, unless explicitly stated otherwise.

## Self-Managed Nodes

A self-managed node is a worker node hosted on a compute instance (or instance pool) that you've created yourself in Compute service, rather than on a compute instance that OKE has created for you. Self-managed nodes are often referred to as Bring Your Own Nodes (BYON). Unlike managed nodes and virtual nodes (which are grouped into managed node pools and virtual node pools respectively), self-managed nodes are not grouped into node pools.
...
For more information, see[Working with Self-Managed Nodes](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengworkingwithselfmanagednodes.htm).

Note that all references to 'nodes' and 'worker nodes' in the OKE documentation cover self-managed nodes, unless explicitly stated otherwise.
...
Kubernetes`ServiceTypes`enable you to specify the kind of service you want to expose. A`LoadBalancer ServiceType`creates an OCI load balancer on load balancer subnets in your VCN.

For more information about services in general, see the[Kubernetes documentation](https://kubernetes.io/docs/concepts/services-networking/service/). For more information about creating load balancer services with OKE, see[Defining Kubernetes Services of Type LoadBalancer](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengcreatingloadbalancer.htm).
...
The Kubernetes version you select when you create a cluster using OKE determines the admission controllers supported by that cluster. To find out the supported admission controllers, the order in which they run in the Kubernetes API server, and the Kubernetes versions in which they are supported, see[Supported Admission Controllers](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Reference/contengadmissioncontrollers.htm).
