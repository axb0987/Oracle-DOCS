...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengusingclusterautoscaler.htm
- Fetched: 2026-09-05 01:57 CDT

# Using the Kubernetes Cluster Autoscaler

Find out how to use the Kubernetes Cluster Autoscaler to automatically resize the managed node pools on a cluster you've created using Kubernetes Engine (OKE).
Note  
  
You cannot use the Kubernetes Cluster Autoscaler with virtual node pools.
...
The Kubernetes Cluster Autoscaler increases or decreases the size of a node pool automatically based on resource requests, rather than on the resource utilization of nodes in the node pool. In addition to scaling based on CPU, memory, and other properties, the Kubernetes Cluster Autoscaler also supports scaling based on requests for GPU accelerators. For example, the Kubernetes Cluster Autoscaler will scale up a node pool when a pod requesting an accelerator cannot be scheduled.
...
- Always have at least one node pool in a cluster that is not managed by the Kubernetes Cluster Autoscaler. This node pool is required to run critcal cluster add-ons, and the Kubernetes Cluster Autoscaler itself. Also note that it is your responsibility to manually scale any node pools that are not managed by the Kubernetes Cluster Autoscaler.
...
For a list of the configuration parameters that are supported when using the Kubernetes Cluster Autoscaler as a cluster add-on, along with the names to use when specifying them, see[Cluster Autoscaler](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/configuration-arguments-cluster-autoscaler.htm).
...
- Does not create additional pods. Adding pods is supported by the Horizontal Pod Autoscaler (see[Using the Kubernetes Horizontal Pod Autoscaler](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengusinghorizontalpodautoscaler.htm)).
- Does not adjust CPU and memory requests and limits for containers. Adjusting limits is supported by the Vertical Pod Autoscaler (see[Using the Kubernetes Vertical Pod Autoscaler](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengusingverticalpodautoscaler.htm)).
- 
