...
- Source: https://docs.oracle.com/iaas/Content/dev/terraform/deprecated-resources.htm
- Fetched: 2026-09-05 19:20 CDT

# Deprecated Resources

Deprecated Oracle Cloud Infrastructure Terraform provider resources.

This topic covers the list of resources and data sources that have been marked deprecated by the Oracle Cloud Infrastructure (OCI) Terraform provider and their respective suggested replacements, if any.

Resources and data sources marked for deprecation trigger warnings during Terraform`plan`and`apply`operations. For example:
...
Deprecation notices for fields can be found in any of the previously released[CHANGELOG](https://github.com/oracle/terraform-provider-oci/blob/master/CHANGELOG.md). Deprecated fields are also shown as deprecated during Terraform`plan`and`apply`operations. For example:

`The 'size_in_mbs' field has been deprecated. Please use 'size_in_gbs' instead.`

Resource Deprecated Argument Suggested Action
`oci_dns_zone``tsig`Use the`tsig_key_id`argument to reference an`oci_dns_tsig_key`resource instead of embedding TSIG keys directly within`external_masters`
