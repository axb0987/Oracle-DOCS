# App Gateway
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/troubleshooting/app_gateway.htm
...
- Verify that the resolver setting in the file`/usr/local/nginx/conf/nginx-cg-sub.conf`is set to the correct IP.
- Verify that the tenant name in`/usr/local/nginx/conf/cloudgate.config`file is configured correctly.
...
Run the`sudo su -`command to sign in as`root`, and when prompted provide the oracle password.
-   

Install telnet by running the following command:
```
...
Run the following telnet command and try to establish a connection to your IAM instance and your application from the App Gateway server.
```
