...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengaboutupgradingclusters.htm
...
For more information about control plane node upgrade, see[Upgrading the Kubernetes Version on Control Plane Nodes in a Cluster](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengupgradingk8smasternode.htm).
...
- When upgrading the Kubernetes version on the control plane, Kubernetes Engine also checks the Kubernetes version configured for each managed node pool. Both the configured node pool version and the Kubernetes versions currently running on worker nodes must be compatible with the target control plane version. If either is outside the permitted version skew, the control plane upgrade fails.
