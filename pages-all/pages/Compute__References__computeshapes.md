# Compute Shapes
- Source: https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm
...
A flexible shape is a shape that lets you customize the number of OCPUs and the amount of memory when launching or resizing your VM. When you[create a VM instance](https://docs.oracle.com/en-us/iaas/Content/Compute/References/../Tasks/launchinginstance.htm)using a flexible shape, you select the number of OCPUs and the amount of memory that you need for the workloads that run on the instance. The network bandwidth and number of VNICs scale proportionately with the number of OCPUs. This flexibility lets you build VMs that match your workload, enabling you to optimize performance and minimize cost.

The flexible shapes are:
- VM.Standard3.Flex (Intel)
- VM.Standard4.Ax.Flex (Intel)
- VM.Standard.E4.Flex (AMD)
- VM.Standard.E5.Flex (AMD)
- VM.Standard.E6.Flex (AMD)
- VM.Standard.E6.Ax.Flex (AMD)
...
- VM.Standard.A4.Ax.Flex (AmpereOne M processor from Ampere)
- VM.DenseIO.E4.Flex (AMD)
- VM.DenseIO.E5.Flex (AMD)
- VM.DenseIO.E6.Ax.Flex (AMD)
...
See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm). 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU, up to 512 GB total

See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm).
VM.Standard4.Ax.Flex 39 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU , up to 360 GB total
VM.Standard.E4.Flex 64

See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm). 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU , up to 1024 GB total

See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm).
VM.Standard.E5.Flex 126
...
126 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU, up to 1454 GB total
VM.Standard.E6.Ax.Flex 94 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU, up to 712 GB total
VM.Standard.A1.Flex 76

(OCPU is 1 core of an Altra processor) 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU, up to 472 GB total
VM.Standard.A2.Flex 78

(OCPU is 2 cores of an AmpereOne processor) 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU , up to 946 GB total
VM.Standard.A4.Flex 45

(OCPU is 2 cores of an AmpereOne M processor) 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU , up to 700 GB total
VM.Standard.A4.Ax.Flex 45

(OCPU is 2 cores of an AmpereOne M processor) 1 GB or a value matching the number of OCPUs, whichever is greater 64 GB per OCPU , up to 720 GB total
...
VM.DenseIO.E5.Flex
- 8 OCPUs , 96 GB memory
- 16 OCPUs , 192 GB memory
- 24 OCPUs , 288 GB memory
- 32 OCPUs , 384 GB memory
- 40 OCPUs , 480 GB memory
- 48 OCPUs , 576 GB memory
...
Custom images are also supported, depending on the image. You must[add flexible shape compatibility to the custom image](https://docs.oracle.com/en-us/iaas/Content/Compute/References/../Tasks/managingcustomimages.htm#Managing_Custom_Images__console-custom-image-tasks), and then test the image on the flexible shape to ensure that it actually works on the shape.

### Supported Regions

For a list of supported regions, see the[compute instance service limits](https://docs.oracle.com/iaas/Content/General/service-limits/default.htm#Compute_Instances). As host capacity becomes available in additional regions, the list is updated.
Note  
...
[Extended memory virtual machine (VM) instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm)are VM instances that provide more memory and cores than available with standard shapes.
...
For bare metal instances, optionally[configure advanced BIOS settings](https://docs.oracle.com/en-us/iaas/Content/Compute/References/../References/bios-settings.htm), such as disabling simultaneous multithreading, disabling cores, or optimizing the NUMA settings.
...
- BM.Standard4.Ax: X12-based standard compute. Processor: Intel Xeon 6987P-C. Base frequency 2.2 GHz, max turbo frequency 3.2 GHz.
...
- BM.Standard.E6.Ax: E6-based standard compute. Processor: AMD EPYC 9J45. Base frequency 2.7 GHz, max boost frequency 4.1 GHz.
...
- BM.Standard.A4.Ax: OCI Ampere A4 Compute Arm-based standard compute. Each OCPU corresponds to two hardware execution threads (2 cores). Processor: Ampere AmpereOne M 192-36M. Frequency 3.6 GHz.
...
BM.Standard4.Ax.120 120 1152 Block storage only 2 x 100 Gbps 256 129 (1 on the first physical NIC, 128 on the second)
...
BM.Standard.E6.256 256 3072

2 x 960 GB NVMe

Block storage 1 x 200 Gbps 256 129 (1 on the first physical NIC, 128 on the second)
BM.Standard.E6.Ax.192 192 1536
...
See[Arm-Based Compute](https://docs.oracle.com/en-us/iaas/Content/Compute/References/arm.htm). 160 1024 Block storage only 2 x 50 Gbps 256 Windows images are not supported on this shape.

BM.Standard.A4.48

See[Arm-Based Compute](https://docs.oracle.com/en-us/iaas/Content/Compute/References/arm.htm). 48 768

1 x 3.84 TB NVMe

Block Storage 1 x 100 Gbps 256 Windows images are not supported on this shape.

BM.Standard.A4.Ax.48
...
- BM.DenseIO.E6.Ax: E6-based dense I/O compute. Processor: AMD EPYC 9J45. Base frequency 2.7 GHz, max boost frequency 4.1 GHz.
...
BM.DenseIO.E5.128 1 128 1536 81.6 TB NVMe SSD Storage (12 x 6.8 TB drives) 1 x 100Gbps 256 129 (1 on the first physical NIC, 128 on the second)
BM.DenseIO.E6.Ax.192 1 192 2304 81.6 TB NVMe SSD Storage (12 x 6.8 TB drives) 1 x 200Gbps 256 129 (1 on the first physical NIC, 128 on the second)
...
BM.GPU.RTXPRO.8
- GPU: 8x NVIDIA RTX PRO 6000 Blackwell GPUs 96 GB GDDR7
- CPU: 2 x 72-core Intel Xeon 6
- 
...
BM.GPU.B300.8
- GPU: 8x NVIDIA B300 Tensor Core GPUs 263 GB
- CPU: 2 x 64-core Intel Emerald Rapids 8592+
- 
...
(GPU: 2xP100) 28 32 192 Block storage only 2 x 25 Gbps 28 15 (1 on the first physical NIC, 14 on the second)

BM.GPU3.8

(GPU: 8xV100) 52 128 768 Block storage only 2 x 25 Gbps 52 27 (1 on the first physical NIC, 26 on the second)

BM.GPU4.8

(GPU: 8xA100) 64 320 2048 27.2 TB NVMe SSD (4 drives)

1 x 50 Gbps

8 x 200 Gbps RDMA 64 Windows images are not supported on this shape.

BM.GPU.A10.4

(GPU: 4xA10) 64 96 1024 7.68 TB NVMe SSD (2 drives) 2 x 50 Gbps 256 Windows images are not supported on this shape.

BM.GPU.A100-v2.8

(GPU: 8xA100) 128 640 2048 27.2 TB NVMe SSD (4 drives)

2 x 50 Gbps

16 x 100 Gbps RDMA 256 Windows images are not supported on this shape.

BM.GPU.MI300X.8 112 1536 2048 8 x 3.84 TB NVMe

1 x 100 Gbps

8 x 1 x 400 Gbps RDMA 256 Windows images are not supported on this shape.

BM.GPU.MI355X.8 128 288 3072 8 x 7.68 TB NVMe

2 x 200 Gbps

8 x 400 Gbps RDMA 256 Windows images are not supported on this shape.

BM.GPU.L40S.4 112 192 1024 2 x 3.84 TB NVMe

1 x 200 Gbps

800 Gbps RDMA 256 Windows images are not supported on this shape.
BM.GPU.RTXPRO.8 144 8 x 96 GB 3072 8 x 7.68 TB NVMe
...
(GPU: 8xH100) 112 640 2048 16 x 3.84 TB NVMe

1 x 100 Gbps

8 x 2 x 200 Gbps RDMA 256 Windows images are not supported on this shape.

BM.GPU.H200.8

(GPU: 8xH200) 112 1128 3072 8 x 3.84 TB NVMe
...
(GPU: 8xB200) 128 1440 4096 8 x 3.84 TB NVMe

2 x 200 Gbps

8 x 400 Gbps RDMA 256 Windows images are not supported on this shape.
BM.GPU.B300.8 128 2100 4096 8 x 3.84 TB NVMe

2 x 200 Gbps

8 x 800 Gbps RDMA 256 Windows images are not supported on this shape.

BM.GPU.GB200.4

(GPU: 4xB200) 144 768 960 4 x 7.68 TB NVMe

2 x 200 Gbps

4 x 400 Gbps RDMA 512 Windows images are not supported on this shape.

BM.GPU.GB300.4

(GPU: 4xB300) 144 1112 960 4 x 7.68 TB NVMe

2 x 200 Gbps

8 x 400 Gbps RDMA 512 Windows images are not supported on this shape.
...
1 X 100 Gbps RDMA 256 129
BM.HPC.E5.144 1 144 768 3.84 TB NVMe SSD (1 drive) 1 x 100Gbps

1 x 100Gbps RDMA 256 129 (1 on the first physical NIC, 128 on the second)
...
- VM.Standard4.Ax: X12-based standard compute. Processor: Intel Xeon 6987P-C. Base frequency 2.2 GHz, max turbo frequency 3.2 GHz.
- 
...
- VM.Standard.E6.Ax: E6-based standard compute. Processor: AMD EPYC 9J45. Base frequency 2.7 GHz, max boost frequency 4.1 GHz.
...
- VM.Standard.A4.Ax: OCI Ampere A4 Compute Arm-based standard compute. Each OCPU corresponds to two hardware execution threads (2 cores). Processor: Ampere AmpereOne M 192-36M. Frequency 3.6 GHz.

Shape OCPU Memory (GB) Local Disk (TB) Max Network Bandwidth Max VNICs Total: Linux Max VNICs Total: Windows

VM.Standard3.Flex

See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible)and[Burstable Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/burstable-instances.htm). 1 minimum, 32 OCPU maximum

See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm). 1 GB minimum, 512 GB maximum

See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm). Block storage only 1 Gbps per OCPU, maximum 32 Gbps
...
See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible). 1 minimum, 39 OCPU maximum 1 GB minimum, 360 GB maximum Block storage only 1 Gbps per OCPU, maximum 99 Gbps
...
See[Always Free Resources](https://docs.oracle.com/iaas/Content/FreeTier/freetier_topic-Always_Free_Resources.htm). 1 Block storage only 480 Mbps 1 -

VM.Standard.E4.Flex

See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible)and[Burstable Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/burstable-instances.htm). 1 OCPU minimum, 64 OCPU maximum

See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm). 1 GB minimum, 1024 GB maximum

See[Extended Memory VM Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/extended-memory-vm-instances.htm). Block storage only 1 Gbps per OCPU , maximum 40 Gbps
...
See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible).

1 OCPU minimum, 126 OCPU maximum

1 GB minimum, 1049 GB maximum Block storage only 1 Gbps per OCPU, maximum 40 Gbps
...
1 GB minimum, 1454 GB maximum Block storage only 1 Gbps per OCPU, maximum 99 Gbps
...
1 GB minimum, 712 GB maximum Block storage only 1 Gbps per OCPU, maximum 99 Gbps
...
See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible)and[Arm-Based Compute](https://docs.oracle.com/en-us/iaas/Content/Compute/References/arm.htm).

1 OCPU minimum, 76 OCPU maximum

See[Always Free Resources](https://docs.oracle.com/iaas/Content/FreeTier/freetier_topic-Always_Free_Resources.htm). 1 GB minimum, 472 GB maximum Block storage only 1 Gbps per OCPU , maximum 40 Gbps

VM with 1 OCPU: 2 VNICs.

VM with 2 or more OCPUs: 1 VNIC per OCPU.

Maximum 24 VNICs. Windows images are not supported on this shape.

VM.Standard.A2.Flex

See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible)and[Arm-Based Compute](https://docs.oracle.com/en-us/iaas/Content/Compute/References/arm.htm). 1 OCPU minimum, 78 OCPU maximum 1 GB minimum , 946 GB maximum Block storage only 1 Gbps per OCPU , maximum 78 Gbps
...
See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible)and[Arm-Based Compute](https://docs.oracle.com/en-us/iaas/Content/Compute/References/arm.htm). 1 OCPU minimum, 45 OCPU maximum 1 GB minimum , 700 GB maximum Block storage only 1 Gbps per OCPU , maximum 100 Gbps
...
See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible)and[Arm-Based Compute](https://docs.oracle.com/en-us/iaas/Content/Compute/References/arm.htm). 1 OCPU minimum, 45 OCPU maximum 1 GB minimum , 720 GB maximum Block storage only 1 Gbps per OCPU , maximum 100 Gbps
...
- VM.DenseIO.E6.Ax: E6-based dense I/O compute. Processor: AMD EPYC 9J45. Base frequency 2.7 GHz, max boost frequency 4.1 GHz.

Shape OCPU Memory (GB) Local Disk (TB) Max Network Bandwidth Max VNICs Total: Linux Max VNICs Total: Windows

VM.DenseIO.E4.Flex

See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible). 8 128 6.8 TB NVMe SSD storage (1 drive) 8 Gbps 8 8
16 256 13.6 TB NVMe SSD storage (2 drives) 16 Gbps 16 16
32 512 27.2 TB NVMe SSD storage (4 drives) 32 Gbps 24 24
VM.DenseIO.E5.Flex 1 8 96 6.8 TB NVMe SSD storage (1 drive) 8 Gbps 8 8
...
VM.DenseIO.E6.Ax.Flex 1 8 96 6.8 TB NVMe SSD storage (1 drive) 8 Gbps 8 8
16 192 13.6 TB NVMe SSD storage (2 drives) 16 Gbps 16 16
24 288 20.4 TB NVMe SSD storage (3 drives) 24 Gbps 24 24
32 384 27.2 TB NVMe SSD storage (4 drives) 32 Gbps 24 24
40 480 34 TB NVMe SSD storage (5 drives) 40 Gbps 24 24
...
(GPU: 1xP100) 12 16 72 Block storage only 8 Gbps 12 12

VM.GPU3.1

(GPU: 1xV100) 6 16 90 Block storage only 4 Gbps 6 6

VM.GPU3.2

(GPU: 2xV100) 12 32 180 Block storage only 8 Gbps 12 12

VM.GPU3.4

(GPU: 4xV100) 24 64 360 Block storage only 24.6 Gbps 24 24

VM.GPU.A10.1

(GPU: 1xA10) 15 24 240 Block storage only 24 Gbps 15 15

VM.GPU.A10.2

(GPU: 2xA10) 30 48 480 Block storage only 48 Gbps 24 24
...
Shape Instance Type Billed OCPU Usable OCPU 1 Total Memory (GB) 3 Usable Memory (GB) 1 Supported Shapes for Hosted VMs
DVH.Standard2.52 2 X7-based VM host 52 48 768 736 VM.Standard2 series
DVH.Standard3.64 X9-based VM host 64 60 1024 960 VM.Standard3 series
DVH.Standard.E2.64 2 E2-based VM host 64 59 512 480 VM.Standard.E2 series
DVH.Standard.E3.128 2 E3-based VM host 128 124 2048 1912 VM.Standard.E3 series
...
DVH.DenseIO2.52 2 X7-based dense I/O VM host 52 48 768 736 VM.DenseIO2 series
...
- For supported VM instances,[change the shape of the instance](https://docs.oracle.com/en-us/iaas/Content/Compute/References/../Tasks/resizinginstances.htm).
- For bare metal instances and VM instances that don't support changing the shape,[terminate the instance but DO NOT delete the boot volume](https://docs.oracle.com/en-us/iaas/Content/Compute/References/../Tasks/terminatinginstance.htm). Then,[use the boot volume to create a new instance](https://docs.oracle.com/en-us/iaas/Content/Compute/References/../Tasks/launchinginstance.htm).
...
Newer shape recommendation: BM.Standard3.64, BM.Standard.E4.128, BM.Standard.E5.192, BM.Standard.E6.256, BM.Standard.A1.160, or BM.Standard.A4.48.
...
Newer shape recommendation: BM.DenseIO.E4.128 or BM.DenseIO.E5.128.
...
51.2 TB NVMe SSD (8 drives) 2 x 25 Gbps 52 total (26 per physical NIC) 27 total (1 on the first physical NIC, 26 on the second)

[BM.HPC2](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#)

Newer shape recommendation: BM.Optimized3.36 or BM.HPC.E5.144.
...
Newer shape recommendation: VM.Standard.E4.Flex, VM.Standard.E5.Flex, VM.Standard.E6.Flex, VM.Standard.A1.Flex, VM.Standard.A2.Flex, or VM.Standard.A4.Flex.
...
See[Flexible Shapes](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm#flexible)and[Burstable Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/References/burstable-instances.htm). 1 OCPU minimum, 64 OCPU maximum 1 GB minimum, 1024 GB maximum Block storage only 1 Gbps per OCPU, maximum 40 Gbps
...
Newer shape recommendation: VM.DenseIO.E4.Flex or VM.DenseIO.E5.Flex.
