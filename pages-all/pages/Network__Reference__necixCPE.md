# NEC IX Series
- Source: https://docs.oracle.com/en-us/iaas/Content/Network/Reference/necixCPE.htm
...
Site-to-Site VPN provides a site-to-site IPSec connection that Oracle Cloud Infrastructure offers for connecting an on-premises network to a Virtual Cloud Network (VCN).
...
The configuration template provided is for an IX3315 running Firmware Ver.10.2.16 or IX2106 running Firmware Ver.10.2.16 software (or later). The template provides information for each tunnel that you must configure. We recommend setting up all configured tunnels for maximum redundancy.

The configuration template refers to these items that you must provide:
- CPE public IP address: The internet-routable IP address assigned to the external interface on the CPE. You or the Oracle administrator provides this value to Oracle when creating the CPE object in the Oracle Console.
...
Configure internal routing for traffic between the CPE and the local network.
- 

Ensure that you allow traffic between the NEC IX Series and the Oracle VCN.
...
You specify the IKE version when defining the IKE gateway. In the following configuration, a comment shows how to configure the IKE gateway for IKEv1 or IKEv2.

[IKEv1 Configuration Template](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/necixCPE.htm#)

[View the IKEv1 configuration template in full screen for easier reading.](https://docs.oracle.com/en-us/iaas/Content/Network/Reference/../non-dita/vpn_cpe_nec_ikev1.txt)
