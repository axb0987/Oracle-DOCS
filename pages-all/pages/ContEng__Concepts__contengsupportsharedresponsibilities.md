...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengsupportsharedresponsibilities.htm
...
Since Kubernetes Engine provides a managed Kubernetes control plane, Oracle performs automated etcd backup every 15 minutes. You cannot access these backups. However, if an event requiring disaster recovery occurs, Oracle uses these backups to restore clusters created by Kubernetes Engine.
...
- CNI plugin ([kube-flannel](https://github.com/flannel-io/flannel)or[VCN Native pod networking](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengpodnetworking_topic-flannel_CNI_plugin.htm))
- [CSI volume plugin](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengcreatingpersistentvolumeclaim_topic-Provisioning_PVCs_on_BV.htm)
...
- Setup and maintain required cluster networking , including VCNs, subnets, gateways, and so on (see[Network Resource Configuration for Cluster Creation and Deployment](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengnetworkconfig.htm)).
...
A core benefit of Kubernetes is its active open source community. The Kubernetes ecosystem encompasses a wide range of projects, each with its own support channels, response times, and intended use cases. When adopting these technologies alongside Kubernetes Engine, make sure you understand the available support options and compatibility of each component.

Oracle maintains the open source components integrated into the Kubernetes Engine control plane. Oracle maintenance includes rigorous build verification, vulnerability scanning, validation testing, and patch management for all container images and binaries distributed by Oracle. For example, Oracle is responsible for the Kubernetes API server and provides support for control plane issues as defined in the Kubernetes Engine[Service Level Agreement (SLA)](https://www.oracle.com/cloud/sla/). Kubernetes Engine add-ons provided by Oracle are fully supported, including installation, uninstallation, lifecycle management, and security patching. For advanced use cases or additional guidance related to specific add-ons, we recommend consulting the upstream project documentation or creating issues in the project's repository.

Kubernetes Engine documentation might reference or recommend open source and third-party tools or software, such as the[Kubernetes Cluster Autoscaler](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengusingclusterautoscaler.htm)and[Istio](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/conteng-using-istio.htm). Deploying and using third-party or open source projects with clusters created using Kubernetes Engine is outside the scope of Oracle’s support agreement. We recommend engaging directly with the respective open source communities and project maintainers to obtain support. For additional information, refer to the[Graduated and Incubating Projects](https://www.cncf.io/projects/)within the Cloud Native Computing Foundation (CNCF).
...
- Versions of Kubernetes beyond those listed in[Supported Versions of Kubernetes](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengaboutk8sversions.htm). If you request support for a cluster running an unsupported version of Kubernetes, you will be asked to upgrade the cluster to a supported version of Kubernetes. To learn more about which versions of Kubernetes are currently supported, refer to[Supported Versions of Kubernetes](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengaboutk8sversions.htm).
...
Oracle is solely responsible for the management of the Kubernetes Engine service.

Responsibility: None
Kubernetes Control Plane

Responsibility: Total
Oracle is solely responsible for the management of:
...
Oracle is responsible for deploying Kubernetes data plane components and add-on software (for example, kubelet, kube-proxy, flannel).
...
Oracle is responsible for providing:
...
You are responsible for monitoring the health of worker nodes using the cluster observability features provided by Oracle.
Backups

Responsibility: Shared

Oracle is responsible for performing an automated etcd backup every 15 minutes.

Responsibility: Shared

You are responsible for backups required by the application.
Disaster Recovery

Responsibility: Shared

If an event requiring disaster recovery occurs, Oracle is responsible for restoring the cluster from the etcd backups.

Responsibility: Shared

You are responsible for restoring data from backups created by the application.
Cluster Networking
...
- Setting up the required cluster networking (see[Network Resource Configuration for Cluster Creation and Deployment](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/contengnetworkconfig.htm)).
...
You are solely responsible for setting up application networking capabilities (for example, load balancers, ingress controllers, and network policies).
...
You are solely responsible for setting up and managing container logs (see[Viewing Application Logs on Managed Nodes and Self-Managed Nodes](https://docs.oracle.com/en-us/iaas/Content/ContEng/Concepts/../Tasks/contengviewingworkernodelogs.htm)) and metrics.
...
You are solely responsible for monitoring the health and performance of applications running on clusters.
...
You are solely responsible for application security.
Application
