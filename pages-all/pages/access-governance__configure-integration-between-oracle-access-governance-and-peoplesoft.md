...
- Source: https://docs.oracle.com/en-us/iaas/Content/access-governance/configure-integration-between-oracle-access-governance-and-peoplesoft.htm
- Fetched: 2026-09-05 03:13 CDT

# Configure Integration Between Oracle Access Governance and PeopleSoft

Configure the orchestrated system, and perform the prerequisites to integrate PeopleSoft with Oracle Access Governance.

Oracle Access Governance supports PeopleSoft Human Capital Management (HCM, PeopleSoft Enterprise Learning Management (ELM), and PeopleSoft Financials and Supply Chain Management (FSCM) both as an authoritative source of identities and a managed system.
...
- Your PeopleSoft instance is certified with Oracle Access Governance. Refer to[Peoplesoft Components Certified for Integration with Oracle Access Governance](https://docs.oracle.com/en-us/iaas/Content/access-governance/peoplesoft-integration-reference.htm#peoplesoft-peoplesoft-components-certified-for-integration-with-oracle-access-gov)for details of the versions supported.
- Your environment requires certain PeopleSoft elements to be present. See[PeopleSoft Components Required For Integration](https://docs.oracle.com/en-us/iaas/Content/access-governance/peoplesoft-integration-reference.htm#peoplesoft-peoplesoft-components-required-for-integration)for details of the requirements.
- Complete the steps in[Configure Oracle Database Schema User Account](https://docs.oracle.com/en-us/iaas/Content/access-governance/peoplesoft-integration-reference.htm#peoplesoft-dbuser).
- Complete the steps in[Configure PeopleSoft Service Account Using Peoplesoft PIA Web Interface](https://docs.oracle.com/en-us/iaas/Content/access-governance/peoplesoft-integration-reference.htm#peoplesoft-serviceuser).
...
- Select which PeopleSoft application you want to connect to. These applications are supported:
...
You configure integration with each application separately. To connect more than one, repeat this configuration process for each application.
- Enter the Easy Connect URL for database for connection to the PeopleSoft Oracle database. For example, for an FSCM connection,`jdbc:oracle:thin:@//<ip_address>:<port>/FSCMPRD`.
- In the Database schema user name enter the name of the schema user you will use to connect to the PeopleSoft Oracle database. For details on how to create this user, refer to[Configure Oracle Database Schema User Account](https://docs.oracle.com/en-us/iaas/Content/access-governance/peoplesoft-integration-reference.htm#peoplesoft-dbuser).
...
- In the PeopleSoft server host and port field, add the JOLT server listener (JSL) host and port. To determine the JSL port of the application server, check the JOLTListener section in the psappsrv.cfg file. The file is located in &lt;PS_CFG_HOME&gt;appserv&lt;DOMAIN_NAME&gt;, where PS_CFG_HOME is the location of the configuration file of the PeopleSoft Server and DOMAIN_NAME is the name of the domain to be administered.
- In the Username field, enter the username required to connect to the PeopleSoft instance to perform data reconciliation and provisioning. For details on how to create this user, refer to[Configure PeopleSoft Service Account Using Peoplesoft PIA Web Interface](https://docs.oracle.com/en-us/iaas/Content/access-governance/peoplesoft-integration-reference.htm#peoplesoft-serviceuser).
...
- In the Custom jar details field enter the details of custom PeopleSoft jars used during integration. The files are`psjoa.jar`and`psmanagement.jar`, and should be entered in the following format:
```
...
For more information on custom jar support, refer to[Custom Jar Support](https://docs.oracle.com/en-us/iaas/Content/access-governance/agent-administration.htm#custom-jar-support).
...
- psjoa.jar
- psmanagement.jar
- Copy the files to the agent custom JARs directory path as specified in the`config.json`, where`/app`is the agent volume. For example, you may have a value such as`"customJarsDirectory": "/app/data/customJars"`in your`config.json`.
Note
