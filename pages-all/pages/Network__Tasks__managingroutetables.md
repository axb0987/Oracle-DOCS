# VCN Route Tables
- Source: https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingroutetables.htm
- Fetched: 2026-09-05 02:45 CDT

# VCN Route Tables

This topic describes how to manage the route tables in a Virtual Cloud Network (VCN). For more on route tables in a Dynamic Routing Gateway (DRG), see[Dynamic Routing Gateways](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingDRGs.htm).
...
- VCN local routing automatically handles traffic between and within the VCN's subnets. Local routing doesn't require defining explicit route rules to enable traffic, local routing rules are implicit and not shown in the routing table. Routing between a VCN's subnets can be changed by adding static routes (see[Intra VCN Routing](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingroutetables.htm#Overview_of_Routing_for_Your_VCN__intra_vcn)).
- You can use[intra-VCN routing](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingroutetables.htm#Overview_of_Routing_for_Your_VCN__intra_vcn)to specify a next-hop private IP, LPG, or DRG within a VCN for traffic destined to another subnet in the VCN. Intra VCN routing lets you create more complex security and network virtualization use cases. OCI also supports Intra VCN routing for traffic entering a VCN through a gateway in addition to traffic between subnets.
...
- Custom Intra VCN routes: These are route rules you create in the VCN or subnet route table for Intra VCN traffic, which can override normal local routes. All custom Intra VCN routes have a target (DRG, LPG, or private IP in the VCN) and a route type of static .
...
- Static routes (user-defined)
- Implicit local routes (created automatically by OCI) not visible in the route table
- IPv6: OCI supports Intra VCN routing for IPv6 VCN prefixes.
Note  
  
Intra subnet routing isn't supported. Traffic with a destination IP address in the same subnet as the originating VNIC is forwarded (not routed) directly to the appropriate destination.
...
- Create static route rules in the IGW route table that specify a next hop of 10.0.1.4 (a firewall) for inbound traffic.
- Create route tables for subnets A, B, and C. Traffic from the internet to subnet B and C must go through the firewall appliance at 10.0.1.4 in subnet A. Traffic between subnet B and C must go through the same firewall.
...
- If the VNIC IP address has an associated own route table, this route table is used to route traffic from the IP address.
- If a VNIC IP address doesn't have its own route table, the VNIC level route table is used to route traffic from the IP address.
- If a VNIC has an associated route able, all IP addresses on the VNIC that don't have an associated route table use the VNIC route table.
- If a VNIC doesn't have an associated route table, all IP addresses on the VNIC that don't have an associated route table use the subnet route table.
Note  
...
Callout A: Custom Route Table A to On-premises
Destination Target Route Type
0.0.0.0/0 DRG Static

Callout B: Custom Route Table B to the Internet
Destination Target Route Type
0.0.0.0/0 IGW Static
...
If you associate a route table with one of these gateways, afterward the gateway must always have an associated route table. The associated route table's rules can be changed or removed. For an internet gateway, the target must be in a public subnet.

A route table associated with an LPG can't use intra VCN routing (a destination in any of the VCN's subnets).
...
If you're not familiar with the definition of a private IP, see[Private IP Addresses](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingIPaddresses.htm). In short: a private IP is an object that contains a private IP address and related properties and has its own[OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm).
...
- Decide which instance you want to receive and forward the traffic.
- Select a private IP on the instance (can be on the instance's primary VNIC or a secondary VNIC). To implement failover, set up a[secondary private IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingIPaddresses.htm)on one of the VNICs on the instance.
- Disable the source/destination check on the private IP's VNIC. See[Overview of VNICs and Physical NICs](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingVNICs.htm#overview).
- Get the OCID for the private IP. If you're using the Console, you can get either the OCID or the private IP address itself, along with the name of the private IP's compartment.
- For the subnet that needs to route traffic to the private IP, view the subnet's route table. If the table already has a rule with the same destination CIDR but a different target, delete that rule.
- 

[Add a route rule](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/update-rules-routetable.htm)with the following:
