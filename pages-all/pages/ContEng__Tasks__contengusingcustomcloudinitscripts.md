...
- Source: https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengusingcustomcloudinitscripts.htm
- Fetched: 2026-09-05 01:57 CDT

# Using Custom Cloud-init Initialization Scripts to Set Up Managed Nodes

Find out how to write custom cloud-init scripts to run on worker nodes in clusters you've created using Kubernetes Engine (OKE).
...
You can use a custom cloud-init script to configure a number of extra options on the[kubelet](https://kubernetes.io/docs/reference/command-line-tools-reference/kubelet/)(the primary node agent) on managed nodes. These extra options are sometimes referred to as`kubelet-extra-args`. Amongst other things, using`kubelet-extra-args`options enables you to:
- configure debug level log verbosity
- use node taints to enforce scheduling policies

Using`kubelet-extra-args`to configure debug level log verbosity:
...
Using`kubelet-extra-args`to add node taints:

To add node taints to enforce scheduling policies, use the following cloud-init script:

```

```

Only pods that have a toleration for both taints (the`mytaint1=true:NoSchedule`taint and the`mytaint2=true:NoSchedule`taint) will be scheduled to run on the worker node.
...
Oracle Linux platform images include the`oci-utils`package. You can use the`[](https://docs.oracle.com/iaas/oracle-linux/oci-utils/index.htm#oci-growfs)oci-growfs`command from that package in a cloud-init script to extend the root partition and then grow the file system.
...
### Example 6: Using a Custom Cloud-init Script to Set Custom Kubernetes Node Names

You can use a custom cloud-init script to set custom Kubernetes node names for managed nodes (in clusters running Kubernetes version 1.32 and later). For example, you might want worker nodes in a managed node pool to register with a name that includes a prefix used by your organization, environment, or application.

Each node name must be unique in the cluster. Use lowercase letters, numbers, and hyphens only, and make sure the generated node name is within the length allowed for Kubernetes node names.

We recommend that you test the script in a non-production node pool before using it in production.

The following example cloud.init script sets custom Kubernetes node names. Consider this script as an example only, and modify it as necessary by adding logic to meet your own requirements. The example script derives a unique suffix from the default OCI hostname and passes the generated node name to kubelet by using the`--hostname-override`option.

```

```

Do not remove or modify the call to the Kubernetes Engine initialization script. The initialization script is required for the worker node to join the cluster correctly.

After creating nodes using the cloud-init script, confirm that the nodes have registered with the expected names by entering:

```

```

The command returns node names with the specified prefix and the unique suffix derived from the default OCI hostname. For example:
```
...
- Create a new script file containing the default logic that Kubernetes Engine provides. You can do this in two ways:
- By selecting the Download option (in the node pool Advanced Options section) when using the Custom Create Cluster , Add Node Pool , or Edit Node Pool dialog. The file you download contains the default logic.
...
Either before or after the default logic provided by Kubernetes Engine, add your own custom logic to the script file. Do not modify the default logic.
...
- Save the custom cloud-init script file you have created.
- Specify the custom cloud-init script file when you create a new cluster, add a new node pool, or modify an existing node pool:
...
- On the Clusters list page, create a new cluster using the 'Custom Create' workflow, or add a new node pool to an existing cluster, or modify an existing node pool. If you need help finding the list page, see[Listing Clusters](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/list-clusters.htm).
- In the node pool Advanced Options section of the Custom Create Cluster , Add Node Pool , or Edit Node Pool dialog (as appropriate), specify:
...
- Create a valid cloud-init file, in one of the formats (for example, cloud-config) and filetypes (for example, a .yaml file) supported by cloud-init. See[Creating a Custom Cloud-init Script](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/contengusingcustomcloudinitscripts.htm#contengusingcustomcloudinitscripts_topic_Creatingthecloudinitscript).
- Open a command prompt and enter one of the following commands to create a new node pool, or update an existing node pool, as appropriate:
- `oci ce node-pool create`
- `oci ce node-pool update`
- As well as the mandatory parameters required by the command you are using:
- Include the`--node-image-id`parameter, even if you do not want to specify a custom image.
- Include the`--node-metadata`optional parameter in the format:
```
