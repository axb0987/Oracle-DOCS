...
The assignment is actually to a[private IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingIPaddresses.htm)object linked to the instance's VNIC. The[VNIC](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingVNICs.htm)that the private IP is assigned to must be in a[public subnet](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Concepts/overview.htm#Public). A given instance can have multiple secondary VNICs, and a given VNIC can have multiple secondary private IP addresses. So you can assign a given instance multiple public IP objects across one or more VNICs if you like.

For an instance to communicate directly with the internet, all of the following are required:
- The instance must be in a[public subnet](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Concepts/overview.htm#Public).
- The instance must have a reserved or ephemeral public IP address.
- The instance's VCN must have an[internet gateway.](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingIGs.htm)
- The public subnet must have[route tables](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingroutetables.htm)and[security lists](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Concepts/securitylists.htm)configured accordingly.
Tip  
  
Oracle Cloud Infrastructure FastConnect public peering lets your on-premises network access the public IP addresses of resources in Oracle Cloud Infrastructure without the traffic traversing the internet . For more information, see[FastConnect](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/../Concepts/fastconnect.htm).
...
To a VNIC's[primary private IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingIPaddresses.htm)only

Limits:
- One per[VNIC](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingVNICs.htm)
- Two per VM instance, and 16 per bare metal instance

To either a primary or[secondary private IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingIPaddresses.htm#overview)

Limit: 65 per[VNIC](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingVNICs.htm)
Creation
...
When you stop an instance, its ephemeral public IPs remain assigned to the instance. You can unassign it at any time, which returns it to your tenancy's pool of reserved public IPs.
...
When you launch an instance in a public subnet, by default, the instance gets a public IP unless you say otherwise. See[Choosing Whether an Ephemeral Public IP Is Assigned at Instance Creation](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/assign-public-ip-instance-launch.htm).
...
For public load balancers, the address can be either a regional reserved public IP address that you create from a pool and assign to the load balancer at creation time, or an ephemeral public IP address assigned by Oracle for the life of the load balancer. When the load balancer is no longer needed, the ephemeral IP address is returned to the pool of available addresses, but the reserved IP address can be moved to a different resource. While active, this public IP appears in the list of your tenancy's reserved public IPs, which[you can view in the Console](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/view-reserved-public-ip.htm).
For NAT gateways, you can choose between a reserved or ephemeral public IP address. By default, Oracle automatically assigns an IP address from its IP pool. See[Creating a NAT Gateway](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/nat-create.htm).
- Ephemeral IP Address : Oracle allocates a temporary IP address from its pool. This is the default option.
- Reserved IP Address : You can either select an existing reserved IP address by name or create a new one. To create a new reserved IP, provide a name and choose a source IP pool; if no pool is selected, the default Oracle IP pool will be used. Alternatively, you can utilize the BYOIP feature to create your own public IP pool and then assign a reserved public IP to your NAT gateway.
...
- [Choosing Whether an Ephemeral Public IP Is Assigned at Instance Creation](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/assign-public-ip-instance-launch.htm)
- [Assigning an Ephemeral Public IP When Creating a Secondary VNIC](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/assign-ephermal-ip-secondary-vnic.htm)
- [Assigning an Ephemeral Public IP to an Existing Primary Private IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/assigning-ephemeral-public-existing-private-ip.htm)
- [Changing the Display Name for an Ephemeral Public IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/change-display-ephermal-ip.htm)
- [Deleting an Ephemeral Public IP From an Instance](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/deleting-ephemeral-public-ip-from-instance.htm)

### Reserved Public IPs
- [Viewing Reserved Public IP Addresses](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/view-reserved-public-ip.htm)
- [Creating a Reserved Public IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/reserved-public-ip-create.htm)
- [Deleting a Reserved Public IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/reserved-public-ip-delete.htm)
...
- [Unassigning a Reserved Public IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/reserved-public-ip-unassign.htm)
- [Reassigning a Reserved Public IP to a Different Private IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/reserved-public-ip-reassign.htm)
- [Changing the Display Name of a Reserved Public IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/reserved-public-ip-change-display-name.htm)
- [Managing Tags for a Reserved Public IP](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/reserved-public-ip-manage-tags.htm)
- [Moving a Reserved Public IP to a Different Compartment](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/reserved-public-ip-move.htm)
