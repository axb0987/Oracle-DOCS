...
- Source: https://docs.oracle.com/en-us/iaas/Content/Identity/policyreference/contengpolicyreference.htm
...
Kubernetes Engine supports all the general variables (see[General Variables](https://docs.oracle.com/en-us/iaas/Content/Identity/policyreference/policyreference_topic-General_Variables_for_All_Requests.htm)), plus the ones listed here.
...
`CreateCluster`(also need`use subnets`,`read virtual-network-family`,`inspect compartments`,`use vnics`,`use network-security-groups`,`use private-ips`, and`manage public-ips`)

`DeleteCluster`(also need`manage cluster-node-pools`,`manage instance-family`,`use subnets`,`use vnics`,`use private-ips`, and`manage public-ips`)

`UpdateClusterEndpointConfig`(also need`use vnics`,`use network-security-groups`,`use private-ips`, and`manage public-ips`)
...
no extra`CreateNodePool`,`DeleteNodePool`, and`UpdateNodePool`(also need`manage instance-family`,`use subnets`,`use vnics`, and`inspect compartments`)
...
CLUSTER_WORK_REQUEST_INSPECT`ListWorkRequests`
...
CLUSTER_WORKLOAD_MAPPING_INSPECT`ListWorkloadMappings`
...
`GetNodePoolOptions`CLUSTER_READ when nodePoolOptionId is a cluster OCID; INSTANCE_INSPECT when nodePoolOptionId=all; COMPARTMENT_INSPECT if compartmentId is specified
